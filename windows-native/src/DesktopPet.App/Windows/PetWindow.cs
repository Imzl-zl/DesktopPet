...
public sealed class PetWindow : Window
{
    ...
    public PetWindow(PetInstance instance, Action<PetWindow, int, int> onDragFinished)
    {
        _instance = instance;
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        BenchTrace($"loaded hwnd=0x{_hwnd:X} hook={(source is null ? "MISSING" : "ok")}");
        DrawFrame(0);
        RestartAnimation();
    }
    ...
    public (int X, int Y) SpriteCenterPhysical
    {
        get
        {
            ...
            var (w, h) = (PlaceholderPet.FrameWidth * scale, PlaceholderPet.FrameHeight * scale);
    ...
    /// <summary>把占位精灵帧绘制到帧缓冲：anchored bottom-center + 整数缩放
    /// （对齐 pet.ts draw()：scale 由帧宽计算、像素风不模糊）。</summary>
