...
function getGlobalNpmRoot() {
  ...
  try {
    cachedGlobalNpmRoot = fs.realpathSync((0, _nodeChild_process.execSync)("npm root -g", { encoding: "utf-8", timeout: 5000 }).trim());
    return cachedGlobalNpmRoot;
  } catch {
...
function collectInstalledPackageSkillPaths(cwd, agentDir) {
  const projectConfigDir = (0, _utils.getProjectConfigDir)(cwd);
  const dirs = [
...
function inferSkillSource(filePath, cwd, agentDir, sourceHint) {
  ...
  const projectConfigRoot = path.resolve((0, _utils.getProjectConfigDir)(cwd));
...
function getCachedSkills(cwd) {
  const now = Date.now();
  const agentDir = (0, _utils.getAgentDir)();
...
localBaseDir)
{
  ...
  for (const name of skillNames) {
    ...
    if (skill) resolved.push(skill);else
    missing.push(trimmed);
  }
