"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.ACCEPTANCE_REPORT_NOT_FOUND = void 0;exports.acceptanceFailureMessage = acceptanceFailureMessage;exports.aggregateAcceptanceReport = aggregateAcceptanceReport;exports.buildSkippedAcceptanceLedger = buildSkippedAcceptanceLedger;exports.evaluateAcceptance = evaluateAcceptance;exports.formatAcceptancePrompt = formatAcceptancePrompt;exports.normalizeAcceptanceInput = normalizeAcceptanceInput;exports.parseAcceptanceReport = parseAcceptanceReport;exports.resolveEffectiveAcceptance = resolveEffectiveAcceptance;exports.stripAcceptanceReport = stripAcceptanceReport;exports.validateAcceptanceInput = validateAcceptanceInput;exports.validateExecutionAcceptance = validateExecutionAcceptance;var _nodeChild_process = await jitiImport("node:child_process");
...

{
  ...
  const intent = (0, _taskIntent.classifyTaskMutationIntent)(input.acceptanceRole ? "worker" : input.agentName, input.task ?? "");
  ...
  const taskMayWrite = readOnlyTask ? false : (0, _taskIntent.taskMayMutate)(input.task ?? "") || intent.kind === "implementation" || rolePatchTask;
...
function validateAcceptanceInput(input, pathLabel = "acceptance") {
  ...
  if (typeof input === "string") {
    if (input === "reviewed") errors.push(`${pathLabel} ${EXPLICIT_REVIEWED_UNAVAILABLE}`);else
    if (!VALID_LEVELS.has(input)) errors.push(`${pathLabel} has invalid level '${input}'.`);else
    if (input === "none") errors.push(`${pathLabel} level "none" requires a reason; use { level: "none", reason: "..." }.`);else
    if (input === "verified") errors.push(`${pathLabel} level "verified" requires object form with at least one verify command.`);
    return errors;
  }
...

{
  ...
  if ((0, _agentContract.isAgentContractV1)(input.agentContract)) {
  ...
  const criteria = normalizeCriteria(
    explicit.criteria?.length ? explicit.criteria : inferred.criteria,
    evidence
  );
...
function formatAcceptancePrompt(acceptance, options = {}) {
...
const ACCEPTANCE_REPORT_NOT_FOUND = exports.ACCEPTANCE_REPORT_NOT_FOUND = "Structured acceptance report not found.";

function parseAcceptanceReport(output) {
...
function stripAcceptanceReport(output) {
...
function checkNoStagedFiles(cwd) {
  const result = (0, _nodeChild_process.spawnSync)("git", ["status", "--short"], { cwd, encoding: "utf-8" });
  if (result.status !== 0) {
...
function aggregateAcceptanceReport(input)
...
function runVerifyCommand(command, defaultCwd, options = {}) {
  return new Promise((resolve) => {
    ...
    let settled = false;
    let hardKill;
    const child = (0, _nodeChild_process.spawn)(command.command, {
      cwd,
    ...
    timeout.unref?.();
    if (options.signal?.aborted) abortVerification();else
...
async function evaluateAcceptance(input)
...
function buildSkippedAcceptanceLedger(acceptance, input) {
...
function acceptanceFailureMessage(ledger) {
