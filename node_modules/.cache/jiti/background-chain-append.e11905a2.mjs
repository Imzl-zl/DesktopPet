"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.appendRunnerStepsToStatus = appendRunnerStepsToStatus;exports.consumeChainAppendRequests = consumeChainAppendRequests;exports.countPendingChainAppendRequests = countPendingChainAppendRequests;exports.enqueueChainAppendRequest = enqueueChainAppendRequest;exports.readPendingChainAppendRequests = readPendingChainAppendRequests;exports.runnerStepOutputNames = runnerStepOutputNames;exports.statusStepDescription = statusStepDescription;var _nodeCrypto = await jitiImport("node:crypto");
var fs = _interopRequireWildcard(await jitiImport("node:fs"));
var path = _interopRequireWildcard(await jitiImport("node:path"));
var _atomicJson = await jitiImport("../../shared/atomic-json.ts");
var _artifacts = await jitiImport("../../shared/artifacts.ts");

var _utils = await jitiImport("../../shared/utils.ts");

var _parallelUtils = await jitiImport("../shared/parallel-utils.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}

const APPEND_REQUESTS_DIR = "append-requests";














function appendDir(asyncDir) {
  return path.join(asyncDir, APPEND_REQUESTS_DIR);
}

function appendRequestPath(asyncDir, request) {
  return path.join(appendDir(asyncDir), `${request.createdAt}-${request.id}.json`);
}

function listAppendRequestFiles(asyncDir) {
  const dir = appendDir(asyncDir);
  try {
    return fs.readdirSync(dir).
    filter((entry) => entry.endsWith(".json")).
    map((entry) => path.join(dir, entry)).
    sort();
  } catch (error) {
    if (error.code === "ENOENT") return [];
    throw error;
  }
}

function countPendingChainAppendRequests(asyncDir) {
  return listAppendRequestFiles(asyncDir).length;
}

function runnerStepOutputNames(steps) {
  const names = [];
  for (const step of steps) {
    if ((0, _parallelUtils.isCheckpointRunnerStep)(step)) {
      continue;
    } else if ((0, _parallelUtils.isParallelGroup)(step)) {
      names.push(...step.parallel.map((task) => task.outputName).filter((name) => Boolean(name)));
    } else if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
      if (step.collect.as) names.push(step.collect.as);
    } else if (step.outputName) {
      names.push(step.outputName);
    }
  }
  return names;
}

function enqueueChainAppendRequest(input)




{
  const status = (0, _utils.readStatus)(input.asyncDir);
  if (!status) throw new Error(`No async run status found for '${input.runId}'.`);
  if (status.runId !== input.runId) throw new Error(`Async run id mismatch: expected '${input.runId}', found '${status.runId}'.`);
  if (status.mode !== "chain") throw new Error(`Run '${input.runId}' is ${status.mode}; only active chain runs accept appended steps.`);
  if (status.state !== "running") throw new Error(`Run '${input.runId}' is ${status.state}; only running chain runs accept appended steps.`);
  const stillInProgress = (status.steps ?? []).some((step) => step.status === "running" || step.status === "pending") || (status.pendingAppends ?? 0) > 0;
  if (!stillInProgress) throw new Error(`Run '${input.runId}' has no running or pending chain steps left; append-step must target an in-progress chain.`);
  if (input.steps.length === 0) throw new Error("append-step requires one chain step.");

  const request = {
    id: (0, _nodeCrypto.randomUUID)(),
    createdAt: input.now ?? Date.now(),
    steps: input.steps
  };
  fs.mkdirSync(appendDir(input.asyncDir), { recursive: true });
  (0, _atomicJson.writeAtomicJson)(appendRequestPath(input.asyncDir, request), request);
  const pendingCount = countPendingChainAppendRequests(input.asyncDir);
  const statusPath = path.join(input.asyncDir, "status.json");
  const updatedStatus = { ...status, pendingAppends: pendingCount, lastUpdate: request.createdAt };
  (0, _atomicJson.writeAtomicJson)(statusPath, updatedStatus);
  (0, _artifacts.appendJsonl)(path.join(input.asyncDir, "events.jsonl"), JSON.stringify({
    type: "subagent.chain.append.requested",
    ts: request.createdAt,
    runId: input.runId,
    requestId: request.id,
    stepCount: input.steps.length,
    pendingAppends: pendingCount
  }));
  return { request, pendingCount };
}

function readAppendRequest(filePath) {
  const raw = JSON.parse(fs.readFileSync(filePath, "utf-8"));
  if (!raw.id || typeof raw.id !== "string") return undefined;
  const createdAt = raw.createdAt;
  if (typeof createdAt !== "number" || !Number.isFinite(createdAt)) return undefined;
  if (!Array.isArray(raw.steps) || raw.steps.length === 0) return undefined;
  return { id: raw.id, createdAt, steps: raw.steps };
}

function readPendingChainAppendRequests(asyncDir) {
  return listAppendRequestFiles(asyncDir).
  map((filePath) => readAppendRequest(filePath)).
  filter((request) => Boolean(request)).
  sort((left, right) => left.createdAt - right.createdAt || left.id.localeCompare(right.id));
}

function consumeChainAppendRequests(asyncDir) {
  const requests = [];
  for (const filePath of listAppendRequestFiles(asyncDir)) {
    const request = readAppendRequest(filePath);
    try {
      fs.unlinkSync(filePath);
    } catch {

      // The runner should not execute a consumed request twice.
    }if (request) requests.push(request);
  }
  return requests.sort((left, right) => left.createdAt - right.createdAt || left.id.localeCompare(right.id));
}

const MAX_STATUS_STEP_DESCRIPTION_CHARS = 160;

/** Bounded one-line per-step task description persisted into status.json for fleet display. */
function statusStepDescription(task) {
  const description = task?.replace(/\s+/g, " ").trim();
  if (!description) return undefined;
  return description.length > MAX_STATUS_STEP_DESCRIPTION_CHARS ?
  `${description.slice(0, MAX_STATUS_STEP_DESCRIPTION_CHARS - 1)}…` :
  description;
}

function statusStepForTask(task) {
  const description = statusStepDescription(task.task);
  return {
    agent: task.agent,
    ...(description ? { description } : {}),
    ...(task.context ? { context: task.context } : {}),
    phase: task.phase,
    label: task.label,
    outputName: task.outputName,
    structured: task.structured,
    status: "pending",
    ...(task.sessionFile ? { sessionFile: task.sessionFile } : {}),
    skills: task.skills,
    model: task.model,
    thinking: task.thinking,
    attemptedModels: task.modelCandidates && task.modelCandidates.length > 0 ? task.modelCandidates : task.model ? [task.model] : undefined,
    recentTools: [],
    recentOutput: []
  };
}

function statusStepForCheckpoint(step, stepIndex) {
  return {
    agent: `checkpoint:${step.checkpoint}`,
    phase: step.phase,
    label: step.label ?? step.checkpoint,
    status: "pending",
    checkpoint: { name: step.checkpoint, ...(step.message ? { message: step.message } : {}), status: "pending", stepIndex },
    recentTools: [],
    recentOutput: []
  };
}

function statusStepsForRunnerStep(step, stepIndex) {
  if ((0, _parallelUtils.isCheckpointRunnerStep)(step)) return [statusStepForCheckpoint(step, stepIndex)];
  if ((0, _parallelUtils.isParallelGroup)(step)) return step.parallel.map(statusStepForTask);
  if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
    return [{
      agent: `expand:${step.parallel.agent}`,
      ...(step.parallel.context ? { context: step.parallel.context } : {}),
      phase: step.phase ?? step.parallel.phase,
      label: step.label ?? step.parallel.label ?? `Dynamic fanout (${step.collect.as})`,
      outputName: step.collect.as,
      structured: Boolean(step.collect.outputSchema),
      status: "pending",
      recentTools: [],
      recentOutput: []
    }];
  }
  return [statusStepForTask(step)];
}

function pushPhase(graph, phase, nodeId) {
  if (!phase) return;
  let group = graph.phases.find((candidate) => candidate.title === phase);
  if (!group) {
    group = { title: phase, nodeIds: [] };
    graph.phases.push(group);
  }
  group.nodeIds.push(nodeId);
}

function graphNodeForSequential(step, stepIndex, flatIndex) {
  return {
    id: `step-${stepIndex}`,
    kind: "step",
    agent: step.agent,
    phase: step.phase,
    label: step.label?.trim() || step.agent || `Step ${stepIndex + 1}`,
    status: "pending",
    flatIndex,
    stepIndex,
    outputName: step.outputName,
    structured: step.structured
  };
}

function graphNodeForParallel(step, stepIndex, flatIndex, graph) {
  const children = step.parallel.map((task, taskIndex) => {
    const childId = `step-${stepIndex}-agent-${taskIndex}`;
    pushPhase(graph, task.phase, childId);
    return {
      id: childId,
      kind: "agent",
      agent: task.agent,
      phase: task.phase,
      label: task.label?.trim() || task.agent || `Agent ${taskIndex + 1}`,
      status: "pending",
      flatIndex: flatIndex + taskIndex,
      stepIndex,
      outputName: task.outputName,
      structured: task.structured
    };
  });
  return {
    id: `step-${stepIndex}`,
    kind: "parallel-group",
    label: step.parallel.length === 1 ? "Parallel task" : `Parallel group (${step.parallel.length})`,
    status: "pending",
    stepIndex,
    children
  };
}

function graphNodeForDynamic(step, stepIndex) {
  return {
    id: `step-${stepIndex}`,
    kind: "dynamic-parallel-group",
    label: step.label?.trim() || step.parallel.label?.trim() || `Dynamic fanout (${step.collect.as})`,
    status: "pending",
    stepIndex,
    outputName: step.collect.as,
    structured: Boolean(step.collect.outputSchema),
    dynamic: {
      sourceOutput: step.expand.from.output,
      sourcePath: step.expand.from.path,
      itemName: step.expand.item ?? "item",
      maxItems: step.expand.maxItems,
      collectAs: step.collect.as
    },
    children: []
  };
}

function appendWorkflowNode(graph, step, stepIndex, flatIndex) {
  if (!graph) return;
  if ((0, _parallelUtils.isCheckpointRunnerStep)(step)) {
    const node = {
      id: `step-${stepIndex}`,
      kind: "checkpoint",
      phase: step.phase,
      label: step.label?.trim() || step.checkpoint,
      status: "pending",
      flatIndex,
      stepIndex,
      checkpoint: { name: step.checkpoint, ...(step.message ? { message: step.message } : {}), status: "pending", stepIndex }
    };
    graph.nodes.push(node);
    pushPhase(graph, step.phase, node.id);
    return;
  }
  if ((0, _parallelUtils.isParallelGroup)(step)) {
    graph.nodes.push(graphNodeForParallel(step, stepIndex, flatIndex, graph));
    return;
  }
  if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
    graph.nodes.push(graphNodeForDynamic(step, stepIndex));
    return;
  }
  const node = graphNodeForSequential(step, stepIndex, flatIndex);
  graph.nodes.push(node);
  pushPhase(graph, step.phase, node.id);
}

function appendRunnerStepsToStatus(input)




{
  let addedChainSteps = 0;
  let addedFlatSteps = 0;
  for (const step of input.steps) {
    const stepIndex = input.status.chainStepCount ?? input.status.steps?.length ?? 0;
    const flatIndex = input.status.steps?.length ?? 0;
    const statusSteps = statusStepsForRunnerStep(step, stepIndex);
    input.status.steps ??= [];
    input.status.steps.push(...statusSteps);
    if ((0, _parallelUtils.isParallelGroup)(step)) {
      input.status.parallelGroups ??= [];
      input.status.parallelGroups.push({ start: flatIndex, count: step.parallel.length, stepIndex });
    } else if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
      input.status.parallelGroups ??= [];
      input.status.parallelGroups.push({ start: flatIndex, count: 1, stepIndex });
    }
    appendWorkflowNode(input.status.workflowGraph, step, stepIndex, flatIndex);
    input.status.chainStepCount = stepIndex + 1;
    addedChainSteps++;
    addedFlatSteps += statusSteps.length;
  }
  input.status.pendingAppends = input.pendingAppends ?? 0;
  input.status.lastUpdate = input.now ?? Date.now();
  return { addedChainSteps, addedFlatSteps };
} /* v9-0f8cd80101e18861 */
