...
public enum PetWindowPlanner {
    ...
    private static func isActive(_ s: ActivityState) -> Bool {
    ...
    public static func plan(activities: [ActivitySession], split: Bool,
                            mappings: [ProjectPetMapping], defaultPetID: String?,
                            forceDefault: Bool = false, hideIdleProjects: Bool = false) -> [PetWindowSpec] {
        let specs = planCore(activities: activities, split: split, mappings: mappings,
        ...
        return specs + [PetWindowSpec(key: defaultKey, projectName: nil, petID: defaultPetID,
                                      activityIDs: [], mood: .idle, count: 0)]
    }

    private static func planCore(activities: [ActivitySession], split: Bool,
                                 mappings: [ProjectPetMapping], defaultPetID: String?,
                                 hideIdleProjects: Bool) -> [PetWindowSpec] {
        let active = activities.filter { isActive($0.state) }

        func defaultWindow(_ a: [ActivitySession]) -> PetWindowSpec {
            PetWindowSpec(key: defaultKey, projectName: nil, petID: defaultPetID,
                          activityIDs: a.map(\.id),
                          mood: a.isEmpty ? .idle : MoodResolver.aggregate(a),
                          count: a.count)
        }
        ...
        // Bucket each active activity under its configured mapping (longest-prefix
        // match), or the default bucket if it belongs to no configured project.
        var byKey: [String: [ActivitySession]] = [:]
        var rest: [ActivitySession] = []
        for s in active {
