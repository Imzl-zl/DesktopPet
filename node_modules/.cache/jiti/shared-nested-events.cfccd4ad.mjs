"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.NESTED_EVENTS_DIR = void 0;exports.applyNestedEvent = applyNestedEvent;exports.assertSafeNestedId = assertSafeNestedId;exports.attachRootChildrenToSteps = attachRootChildrenToSteps;exports.buildNestedRouteIndex = buildNestedRouteIndex;exports.createNestedRoute = createNestedRoute;exports.findNestedControlResult = findNestedControlResult;exports.findNestedRouteForRootId = findNestedRouteForRootId;exports.findNestedRun = findNestedRun;exports.findNestedRunById = findNestedRunById;exports.findNestedRunMatchesById = findNestedRunMatchesById;exports.hasLiveNestedDescendants = hasLiveNestedDescendants;exports.isSafeNestedId = isSafeNestedId;exports.isTopLevelAsyncDir = isTopLevelAsyncDir;exports.nestedArtifactEnv = nestedArtifactEnv;exports.nestedResultsPath = nestedResultsPath;exports.nestedRouteEnv = nestedRouteEnv;exports.nestedSummaryFromAsyncStatus = nestedSummaryFromAsyncStatus;exports.parseNestedEventRecords = parseNestedEventRecords;exports.projectNestedEvents = projectNestedEvents;exports.projectNestedRegistryForRoot = projectNestedRegistryForRoot;exports.readNestedControlRequests = readNestedControlRequests;exports.readNestedControlResults = readNestedControlResults;exports.readNestedRegistry = readNestedRegistry;exports.resolveInheritedNestedRouteFromEnv = resolveInheritedNestedRouteFromEnv;exports.resolveNestedAsyncDir = resolveNestedAsyncDir;exports.resolveNestedParentAddressFromEnv = resolveNestedParentAddressFromEnv;exports.resolveNestedRouteFromEnv = resolveNestedRouteFromEnv;exports.sanitizeSummary = sanitizeSummary;exports.snapshotNestedEventFiles = snapshotNestedEventFiles;exports.updateAsyncJobNestedProjection = updateAsyncJobNestedProjection;exports.updateForegroundNestedProjection = updateForegroundNestedProjection;exports.writeNestedControlRequest = writeNestedControlRequest;exports.writeNestedControlResult = writeNestedControlResult;exports.writeNestedEvent = writeNestedEvent;var _nodeCrypto = await jitiImport("node:crypto");
...
var _piArgs = await jitiImport("./pi-args.ts");
...
var _processTerminal = await jitiImport("../background/process-terminal.ts");
var _modelInfo = await jitiImport("../../shared/model-info.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}

const NESTED_EVENTS_DIR = exports.NESTED_EVENTS_DIR = path.join(_types.TEMP_ROOT_DIR, "nested-subagent-events");
...
function isSafeNestedId(value) {
  return (0, _nestedPath.isSafeNestedPathId)(value);
}

function assertSafeNestedId(label, value) {
...
function createNestedRoute(rootRunId) {
  assertSafeId("rootRunId", rootRunId);
  const capabilityToken = (0, _nodeCrypto.randomUUID)();
...
function resolveNestedRouteFromEnv(env = process.env) {
  const rootRunId = env[_piArgs.SUBAGENT_PARENT_ROOT_RUN_ID_ENV];
  const eventSink = env[_piArgs.SUBAGENT_PARENT_EVENT_SINK_ENV];
  const controlInbox = env[_piArgs.SUBAGENT_PARENT_CONTROL_INBOX_ENV];
  const capabilityToken = env[_piArgs.SUBAGENT_PARENT_CAPABILITY_TOKEN_ENV];
...
function resolveInheritedNestedRouteFromEnv(env = process.env) {
  try {
...
function resolveNestedParentAddressFromEnv(env = process.env) {
  const parentRunId = env[_piArgs.SUBAGENT_PARENT_RUN_ID_ENV];
  if (!isSafeNestedId(parentRunId)) return undefined;
  const rawIndex = env[_piArgs.SUBAGENT_PARENT_CHILD_INDEX_ENV];
  const parentStepIndex = rawIndex && /^\d+$/.test(rawIndex) ? Number(rawIndex) : undefined;
  const depth = Math.min(Math.max(1, clampNumber(Number(env[_piArgs.SUBAGENT_PARENT_DEPTH_ENV])) ?? 1), MAX_DEPTH);
  const parsedPath = (0, _nestedPath.parseNestedPathEnv)(env[_piArgs.SUBAGENT_PARENT_PATH_ENV]);
...
function resolveNestedAsyncDir(rootRunId, run) {
  if (!run.asyncDir) return undefined;
  const resolved = path.resolve(run.asyncDir);
  const nestedRoot = path.resolve(_types.TEMP_ROOT_DIR, "nested-subagent-runs", rootRunId, run.id);
...
function sanitizeStep(input, depth) {
  ...
  const thinking = _modelInfo.THINKING_LEVELS.find((level) => level === raw.thinking);
  return {
    agent,
    status,
    ...(model ? { model } : {}),
...
function sanitizeSummary(input, depth = 0) {
  if (!input || typeof input !== "object") return undefined;
  const raw = input;
  if (!isSafeNestedId(raw.id) || !isSafeNestedId(raw.parentRunId)) return undefined;
  const pathParts = (0, _nestedPath.sanitizeNestedPath)(raw.path);
  ...
  return {
    ...
    ...(_modelInfo.THINKING_LEVELS.find((level) => level === raw.thinking) ? { thinking: _modelInfo.THINKING_LEVELS.find((level) => level === raw.thinking) } : {}),
...
function parseNestedEventRecords(content, route) {
  if (!content.includes("\n")) {
...
function mergeSummary(existing, event) {
  ...
  incomingSteps[index] ? mergeStepSummary(existingSteps[index], incomingSteps[index]) : existingSteps[index]).filter((step) => Boolean(step));
  const children = mergeBoundedChildren(existing.children, incoming.children);
  return {
    ...existing,
    ...incoming,
...
function findNestedRouteForRootId(rootRunId) {
  assertSafeId("rootRunId", rootRunId);
  let entries;
  try {
 ...
 */
function buildNestedRouteIndex() {
  let entries;
  try {
    entries = fs.readdirSync(NESTED_EVENTS_DIR);
  } catch (error) {
...
function findNestedRunMatchesById(id, options = {}) {
  assertSafeId("id", id);
  const matches = [];
...
function projectNestedEvents(route) {
  validateRouteShape(route);
  ...
  if (changed) {
    ...
    (0, _atomicJson.writeAtomicJson)(registryPath(route), registry);
...
function writeRouteRecord(dir, ts, payload) {
  ...
  const name = `${String(ts).padStart(13, "0")}-${(0, _nodeCrypto.randomUUID)()}.json`;
...
function writeNestedEvent(route, event) {
  validateRouteShape(route);
  const record = {
    ...event,
    rootRunId: route.rootRunId,
...
function writeNestedControlRequest(route, request) {
  validateRouteShape(route);
...
function readNestedControlRequests(route) {
  validateRouteShape(route);
  let entries = [];
  try {
...
function writeNestedControlResult(route, result) {
  validateRouteShape(route);
...
function snapshotNestedEventFiles(route) {
  return new Set(listNestedEventFiles(route));
}

function findNestedControlResult(route, requestId, targetRunId, ignoredFiles) {
...
function readNestedControlResults(route) {
  return listNestedEventFiles(route).
  sort().
  flatMap((entry) => readControlResultsFromFile(route, path.join(route.eventSink, entry)));
}

function nestedRouteEnv(route) {
  return {
    [_piArgs.SUBAGENT_PARENT_EVENT_SINK_ENV]: route.eventSink,
    [_piArgs.SUBAGENT_PARENT_CONTROL_INBOX_ENV]: route.controlInbox,
    [_piArgs.SUBAGENT_PARENT_ROOT_RUN_ID_ENV]: route.rootRunId,
    [_piArgs.SUBAGENT_PARENT_CAPABILITY_TOKEN_ENV]: route.capabilityToken
  };
}

function attachRootChildrenToSteps(rootRunId, steps, children) {
...
function updateAsyncJobNestedProjection(job) {
...
function updateForegroundNestedProjection(control) {
...
function hasLiveNestedDescendants(children) {
...
function nestedSummaryFromAsyncStatus(status, asyncDir, fallback) {
  return {
    ...
    ...(status.processTerminal ? { processTerminal: (0, _processTerminal.sanitizeProcessTerminal)(status.processTerminal, { runId: status.runId || fallback.id, runnerProcessInstanceId: status.processTerminal.runnerProcessInstanceId }, `${asyncDir}/status.json`) } : {}),
    ...
    ...(status.steps?.length ? { steps: status.steps.map((step, index) => ({
        ...
        ...(step.processTerminal ? { processTerminal: (0, _processTerminal.sanitizeProcessTerminal)(step.processTerminal, { runId: status.runId || fallback.id, runnerProcessInstanceId: step.processTerminal.runnerProcessInstanceId }, `${asyncDir}/status.json step ${index}`) } : {}),
...
function nestedArtifactEnv(rootRunId, parentRunId) {
  return {
...
function isTopLevelAsyncDir(asyncDir) {
  const resolved = path.resolve(asyncDir);
  return containedPath(_types.DIRS.async, resolved) && !containedPath(path.join(_types.TEMP_ROOT_DIR, "nested-subagent-runs"), resolved);
}

function nestedResultsPath(rootRunId, id) {
  assertSafeId("rootRunId", rootRunId);
  assertSafeId("id", id);
  return path.join(_types.DIRS.results, "nested", rootRunId, `${id}.json`);
} /* v9-b74cf35686595e05 */
