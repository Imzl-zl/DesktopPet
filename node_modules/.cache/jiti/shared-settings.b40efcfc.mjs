...
// Chain Step Types
// =============================================================================
/** Sequential step: single agent execution */ /** Parallel task item within a parallel step */ /** Approval checkpoint: pause before later chain steps until parent approval. */ /** Parallel step: multiple agents running concurrently */ /** Union type for chain steps */ // =============================================================================
// Type Guards
// =============================================================================
function isCheckpointStep(step) {return "checkpoint" in step;}function isParallelStep(step) {return "parallel" in step && Array.isArray(step.parallel);}function isDynamicParallelStep(step) {return "expand" in step && "collect" in step && "parallel" in step && !Array.isArray(step.parallel);} /** Get all agent names in a step (single for sequential, multiple for parallel) */function getStepAgents(step) {if (isCheckpointStep(step)) {return [];}if (isParallelStep(step)) {return step.parallel.map((t) => t.agent);}if (isDynamicParallelStep(step)) {return [step.parallel.agent];}return [step.agent];} // =============================================================================
 ...
 */function resolveChainTemplates(steps) {return steps.map((step, i) => {if (isCheckpointStep(step)) return "";if (isParallelStep(step)) {// Parallel step: resolve each task's template
        return step.parallel.map((task) => {if (task.task) return task.task; // Default for parallel tasks is {previous}
            return "{previous}";});}if (isDynamicParallelStep(step)) {return step.parallel.task ?? "{previous}";} // Sequential step: existing logic
 ...
 */function writeInitialProgressFile(progressDir) {fs.mkdirSync(progressDir, { recursive: true });fs.writeFileSync(path.join(progressDir, "progress.md"), INITIAL_PROGRESS_CONTENT);}function buildChainInstructions(behavior, chainDir, isFirstProgressAgent, previousSummary) {const prefixParts = [];const suffixParts = []; // READS - prepend to override any hardcoded filenames in task text
 ...
 */function createParallelDirs(chainDir, stepIndex, taskCount, agentNames) {for (let i = 0; i < taskCount; i++) {const subdir = path.join(chainDir, `parallel-${stepIndex}`, `${i}-${agentNames[i]}`);fs.mkdirSync(subdir, { recursive: true });}} /* v9-c9cfb59479437160 */
