"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.PI_AGGREGATE_EVENT_PROJECTOR = exports.MAX_CHILD_STDERR_BYTES = exports.MAX_CHILD_PENDING_LINE_BYTES = void 0;exports.createBoundedByteTail = createBoundedByteTail;exports.createBoundedLineReader = createBoundedLineReader;exports.formatProtocolOutputLimit = formatProtocolOutputLimit;exports.projectChildLifecycle = projectChildLifecycle;var _nodeBuffer = await jitiImport("node:buffer");




const MAX_CHILD_PENDING_LINE_BYTES = exports.MAX_CHILD_PENDING_LINE_BYTES = 16 * 1024 * 1024;
const MAX_CHILD_STDERR_BYTES = exports.MAX_CHILD_STDERR_BYTES = 128 * 1024;
...
function createPiAggregateProjection() {
  ...
  const isTopLevelField = (key) => stack.length === 1 && parent()?.type === "object" && (key === "type" || key === "willRetry");
  ...
  const startValue = (char) => {
    ...
    if (char === "t") token = { type: "literal", expected: "true", index: 1, value: true };else
    if (char === "f") token = { type: "literal", expected: "false", index: 1, value: false };else
    if (char === "n") token = { type: "literal", expected: "null", index: 1, value: null };else
    if (char === "-") token = { type: "number", phase: "minus" };else
    if (char === "0") token = { type: "number", phase: "zero" };else
    if (char >= "1" && char <= "9") token = { type: "number", phase: "int" };else
    return false;
    return true;
  };
  ...
  const processChar = (char) => {
    ...
    if (token?.type === "number") {
      ...
      if (phase === "minus") {
        if (char === "0") token.phase = "zero";else
        if (char >= "1" && char <= "9") token.phase = "int";else
        return false;
        return true;
      }
 ...
 */
const PI_AGGREGATE_EVENT_PROJECTOR = exports.PI_AGGREGATE_EVENT_PROJECTOR = {
  accepts(prefix) {
...
function formatProtocolOutputLimit(limit) {
...
function createBoundedLineReader(options)
...

{
  ...
  let pending = [];
  let pendingBytes = 0;
  let projectedPrefix = _nodeBuffer.Buffer.alloc(0);
  let projectedTail = _nodeBuffer.Buffer.alloc(0);
  let projectedBytes = 0;
  let projection;
  let projectingOversizedLine = false;
  let limitExceeded = false;

  const diagnosticTail = (prior, segment) => {
    ...
    tailFromSegment :
    _nodeBuffer.Buffer.concat([prior.subarray(Math.max(0, prior.length - (MAX_PROTOCOL_DIAGNOSTIC_BYTES - tailFromSegment.length))), tailFromSegment]);
  };

  const failLimit = (observedBytes, prefix, tail) => {
    limitExceeded = true;
    pending = [];
    pendingBytes = 0;
    projectingOversizedLine = false;
    projection = undefined;
    projectedPrefix = _nodeBuffer.Buffer.alloc(0);
    projectedTail = _nodeBuffer.Buffer.alloc(0);
    projectedBytes = 0;
    options.onLimit({
  ...
  const finishLine = () => {
    if (projectingOversizedLine) {
      ...
      options.onLine(_nodeBuffer.Buffer.concat(pending, pendingBytes).toString("utf8"));
    }
    pending = [];
    pendingBytes = 0;
    projectingOversizedLine = false;
  ...
  const append = (segment) => {
    ...
    if (observedBytes > maxPendingLineBytes) {
      const prior = pendingBytes > 0 ? _nodeBuffer.Buffer.concat(pending, pendingBytes) : _nodeBuffer.Buffer.alloc(0);
      ...
      prefixFromPrior :
      _nodeBuffer.Buffer.concat([prefixFromPrior, segment.subarray(0, MAX_PROTOCOL_DIAGNOSTIC_BYTES - prefixFromPrior.length)]);
  ...
  return {
    push(chunk) {
      ...
      const bytes = typeof chunk === "string" ? _nodeBuffer.Buffer.from(chunk) : chunk;
      let start = 0;
...
function createBoundedByteTail(maxBytes = MAX_CHILD_STDERR_BYTES)



{
  if (!Number.isInteger(maxBytes) || maxBytes < 1) throw new Error("maxBytes must be a positive integer.");
  let tail = _nodeBuffer.Buffer.alloc(0);
  return {
    push(chunk) {
      const bytes = typeof chunk === "string" ? _nodeBuffer.Buffer.from(chunk) : chunk;
      tail = trimToUtf8Boundary(_nodeBuffer.Buffer.concat([tail, bytes]), maxBytes);
    },
...
function projectChildLifecycle(event, terminalAssistantStop = false) {
