"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.computeMcpServerHash = computeMcpServerHash;exports.resolveMcpDirectToolNames = resolveMcpDirectToolNames;exports.resolveMcpDirectToolSelections = resolveMcpDirectToolSelections;var _nodeCrypto = await jitiImport("node:crypto");
...
function resolveMcpDirectToolSelections(mcpDirectTools, cwd = process.cwd()) {
...
function loadMetadataCache() {
  const cachePath = path.join((0, _utils.getAgentDir)(), "mcp-cache.json");
  let parsed;
  try {
...
function getConfigPaths(cwd) {
  const piGlobalPath = path.join((0, _utils.getAgentDir)(), "mcp.json");
  const projectPath = path.resolve(cwd, ".mcp.json");
  const projectPiPath = path.resolve((0, _utils.getProjectConfigDir)(cwd), "mcp.json");
  const sources = [];
...
function resolveDirectToolSelections(config, cache, prefix, envOverride) {
  const names = [];
...
function resolveMcpDirectToolNames(mcpDirectTools, cwd = process.cwd()) {
...
function computeMcpServerHash(definition) {
  const identity = {
  ...
  return (0, _nodeCrypto.createHash)("sha256").update(stableStringify(identity)).digest("hex");
}
