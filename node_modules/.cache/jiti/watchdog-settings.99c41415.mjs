"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.DEFAULT_WATCHDOG_CONFIG = void 0;exports.getWatchdogProjectSettingsPath = getWatchdogProjectSettingsPath;exports.getWatchdogUserSettingsPath = getWatchdogUserSettingsPath;exports.resolveWatchdogConfig = resolveWatchdogConfig;exports.resolveWatchdogConfigStrict = resolveWatchdogConfigStrict;exports.writeUserWatchdogEnabled = writeUserWatchdogEnabled;exports.writeWatchdogModelSettings = writeWatchdogModelSettings;var fs = _interopRequireWildcard(await jitiImport("node:fs"));
...
const DEFAULT_WATCHDOG_CONFIG = exports.DEFAULT_WATCHDOG_CONFIG = {
  enabled: false,
  delivery: "held",
  showDuringRun: false,
  syncBacklog: "off",
...
function parseThinking(value, field, meta) {
  ...
  if (typeof value === "string" && _modelInfo.THINKING_LEVELS.includes(value)) return value;
  throw invalid(meta, field, `${_modelInfo.THINKING_LEVELS.map((level) => `'${level}'`).join(" or ")} or false`);
}
...
function parseSyncBacklog(value, field, meta) {
...
function parseAutoFollowPatch(value, field, meta) {
  const input = parseObject(value, field, meta);
...
function parseAsyncCompletionPatch(value, field, meta) {
  const input = parseObject(value, field, meta);
...
function parseWatchdogPatch(value, field, meta) {
  ...
  const patch = {};
  if ("enabled" in input) patch.enabled = parseBoolean(input.enabled, `${field}.enabled`, meta);
  if ("delivery" in input) patch.delivery = parseEnum(input.delivery, `${field}.delivery`, meta, _types.WATCHDOG_DELIVERY_MODES);
  ...
  if ("lateWarningPolicy" in input) {
    patch.lateWarningPolicy = parseEnum(input.lateWarningPolicy, `${field}.lateWarningPolicy`, meta, _types.WATCHDOG_LATE_WARNING_POLICIES);
  }
  if ("severityThreshold" in input) {
    patch.severityThreshold = parseEnum(input.severityThreshold, `${field}.severityThreshold`, meta, _types.WATCHDOG_WARNING_SEVERITIES);
  }
...
function getUserSettingsPath() {
  return path.join((0, _utils.getAgentDir)(), "settings.json");
}
...
function getProjectSettingsPath(cwd) {
  let currentDir = cwd;
  while (true) {
    if (isDirectory((0, _utils.getProjectConfigDir)(currentDir)) || isDirectory(path.join(currentDir, ".agents"))) {
      return path.join((0, _utils.getProjectConfigDir)(currentDir), "settings.json");
    }
...
function getWatchdogProjectSettingsPath(cwd) {
  return path.join((0, _utils.getProjectConfigDir)(cwd), "settings.json");
}
...
function resolveWatchdogConfigStrict(cwd, options = {}) {
  let patch = {};
...
function writeWatchdogModelSettings(input) {
  ...
  if (input.model === null) delete target.model;else
  if (input.model !== undefined) target.model = input.model;
  if (input.thinking === null) delete target.thinking;else
  if (input.thinking !== undefined) target.thinking = input.thinking;
  return writeSettingsFile(settingsPath, settings);
}

function resolveWatchdogConfig(cwd, options = {}) {
  const sources = [];
  const errors = [];
  let patch = {};
  const sourceSpecs = [
