...
/// (finished work, activity snacks) always goes to the currently selected
...
final class PetCareController: ObservableObject {
    ...
    private static let storageKey = "desktoppet.care.v2"
    private static let legacyKey = "desktoppet.care.v1"
    ...
    init() {
        if let data = UserDefaults.standard.data(forKey: Self.storageKey),
           let saved = try? JSONDecoder().decode([String: PetCareState].self, from: data) {
            ...
            if let id = UserDefaults.standard.string(forKey: "desktoppet.selectedPetID") {
                states = [id: old]
                persist()
            }
    ...
    /// A finished piece of work — the pet's proper meal.
    ...
    /// Tokens consumed by a turn (kept for legacy token-fed pets; the pure-pet
    /// path is `feed(activity:)`).
    /// Pass `petID` to feed a specific pet (Split ON); `nil` feeds the selected pet (back-compat).
    ...
    /// Feeds the pet from a completed activity (a chat turn, a daily summary, ...).
    /// Events with weight ≥ 0.5 count as a proper meal (hunger resets); lighter
    /// events give a small XP snack proportional to their weight.
    func feed(activity: ActivityEvent) {
        guard activity.weight > 0 else { return }
        mutate(petID: nil) { state in
            if activity.weight >= 0.5 {
                PetCare.recordMeal(state: &state, now: activity.timestamp)
            } else {
                PetCare.addActivityXP(max(1, Int(activity.weight * 10)),
                                      state: &state, now: activity.timestamp)
            }
        }
    }
...
/// Care-driven chatter: hunger colours the idle pool.
enum CareChat {
    static let hungry = [
        "Getting hungry… feed me something?",
        "A little snack? One small task?",
        "My tummy is rumbling…",
        "Feed me a little XP, please.",
    ]

    static let starving = [
        "Starving… nothing to eat for days…",
        "So weak… one tiny snack, please…",
        "No food… no energy…",
