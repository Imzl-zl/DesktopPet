...
/// Resolves the aggregate activity mood, plays a short `celebrate` burst when
...
final class PetController: ObservableObject {
    ...
    @Published var splitPet: Bool = UserDefaults.standard.bool(forKey: "desktoppet.splitPet") {
        didSet {
            UserDefaults.standard.set(splitPet, forKey: "desktoppet.splitPet")
            update(activities: latestActivities)   // re-evaluate windows when toggled
        }
    }
    ...
    @Published var hideIdleProjectPets: Bool = UserDefaults.standard.bool(forKey: "desktoppet.hideIdleProjectPets") {
        didSet {
            UserDefaults.standard.set(hideIdleProjectPets, forKey: "desktoppet.hideIdleProjectPets")
            update(activities: latestActivities)
        }
    }
    ...
    @Published var animationsEnabled: Bool =
        (UserDefaults.standard.object(forKey: "desktoppet.animationsEnabled") as? Bool) ?? true {
        didSet { UserDefaults.standard.set(animationsEnabled, forKey: "desktoppet.animationsEnabled") }
    }
    ...
    @Published var animationFPS: Double =
        ((UserDefaults.standard.object(forKey: "desktoppet.animationFPS") as? Double) ?? 8).clampedFPS {
        didSet {
            let v = animationFPS.clampedFPS
            if v != animationFPS { animationFPS = v; return }   // re-entrancy guard for clamp
            UserDefaults.standard.set(animationFPS, forKey: "desktoppet.animationFPS")
        }
    }
    ...
    private var latestActivities: [ActivitySession] = []

    private static let petKey = "desktoppet.selectedPetID"
    private static let chatKey = "desktoppet.showChat"
    private static let idleMsgKey = "desktoppet.showIdleMessage"
    private static let sizeKey = "desktoppet.petSize"
    ...
    func start() {
        ...
        ProjectPetSettings.shared.onChange = { [weak self] in
            ...
            self.update(activities: self.latestActivities)
        }
    }
    ...
    /// Called by the app layer whenever the activity list changes.
    func update(activities: [ActivitySession]) {
        latestActivities = activities
        let resolved = MoodResolver.aggregate(activities)
    ...
    private func chatLine(forMood mood: PetMood) -> String {
        ChatSettings.shared.lines(for: mood).randomElement() ?? ""
    }

    /// The chat line shown for a planned window. For split mode the per-window
    /// line is picked from the mood pools; the single-pet path reuses the
    /// aggregate `chatLine`.
    private func chatLine(forMood mood: PetMood, petID: String? = nil) -> String {
        switch mood {
        case .idle, .sleepy:
            guard showIdleMessage else { return "" }
            return idleLine(petID: petID)
        case .working, .waiting, .done, .celebrate, .levelup:
    ...
    /// Plans the per-project pet windows from the current activities and reconciles
        ...
        PetWindowController.shared.sync(specs: specs) { [weak self] spec in
            self?.windowState(for: spec)
                ?? PetWindowController.WindowState(petID: spec.petID, mood: spec.mood,
                                                   activities: [], count: spec.count, chatLine: "")
        }
    }
    ...
    private func windowState(for spec: PetWindowSpec) -> PetWindowController.WindowState {
        ...
        // user, not the work. `.sleepy` is the rest visual; `.idle` on perk-up.
        if spec.key == PetWindowPlanner.defaultKey, let bs = breakState {
            ...
            return PetWindowController.WindowState(
                petID: petID, mood: mood, activities: [], count: 0, chatLine: line)
        }

        if let flash = celebratingKeys[spec.key] {
            return PetWindowController.WindowState(
                petID: petID, mood: flash.mood, activities: [], count: spec.count, chatLine: flash.line
            )
        }

        let ids = Set(spec.activityIDs)
        let groupActivities: [ActivitySession] = latestActivities
            .filter { ids.contains($0.id) && $0.state != .idle }
            .sorted { lhs, rhs in
        ...
        let resolvedLine = chatLine(forMood: spec.mood, petID: spec.petID)
        return PetWindowController.WindowState(
            petID: petID,
            mood: spec.mood,
