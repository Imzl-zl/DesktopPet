...
- `.tasks/windows-important-hardening` child 5 待执行：真实 Windows 验收（托盘可见性/GraphicsCapture 分辨率变化/弹幕防追尾视觉/GPU/Win32/Credential Manager/日志导出/恢复出厂重启/多屏 mixed-DPI）。`windows-review-fix2`（2026-08-11）审查发现修复已闭环，真机 smoke 待补。
...
## 最近活跃窗口
- 2026-08-11（审查修复批 2，`.tasks/windows-review-fix2` 21/21 闭环）：新增 10 回归测试，637 tests 全绿 + sln build 0 warn/error。核心修复：① 托盘图标从未创建（H.NotifyIcon 2.1.4 代码创建必须 `ForceCreate()`，否则 Shell_NotifyIcon 不触发——托盘退出/设置入口整体失效，反射+IL 实证）；② Scheduler P0 预留槽位（worker 数=concurrency+1，P0 立即执行）+ 排队预算（入队起算，超时由队列看门狗清理）；③ InteractionEngine 连续活动判定改「最新事件语义」（原用最早事件，切走/离开误报）；④ FileJsonStore 损坏隔离（.corrupt-* + FileCorrupted 事件 + 弹窗提示，不再静默覆盖）；⑤ AgentHost 关闭顺序（InvokeShutdown 前释放 capture，原每次正常退出走 FATAL 路径）；⑥ GraphicsCapture 分辨率变化重建帧池（FrameArrived 内比较 item.Size，GraphicsCaptureItem 只有 Closed 事件无 SizeChanged）；⑦ 热键循环含 bubble（原回不到默认模式）；⑧ 浮球右键菜单加退出项；⑨ MaxOutputTokens/ContextTokens 去掉跨连接回退（串配置）；⑩ 事件活性看门狗（10min 无事件重启 Agent）；⑪ PushConfig 拥塞不销毁连接；⑫ runtime 重建签名比较（改无关设置不再重建 scheduler 池）；⑬ AsyncGenerationOwner Dispose 超时强制回收（防退出挂死）；⑭ EdgeTts.cs 移出生产程序集（挪到 Infra.Tests 作协议知识库）；⑮ DanmakuEngine 防追尾重构（tail 跟踪当前 x，宽屏下原限流死代码+全部弹幕挤轨道 0）。待真机验证：托盘图标可见性（用户确认）、分辨率变化捕获、弹幕防追尾视觉效果。
