...
final class PetCareTests: XCTestCase {
    ...
    // MARK: - Activity XP (pure-pet feeding)

    func testAddActivityXPGrantsXPAndMarksFed() {
        var state = PetCareState()
        let now = date("2026-01-01 10:00")
        XCTAssertEqual(PetCare.addActivityXP(5, state: &state, now: now), 5)
        XCTAssertEqual(state.xp, 5)
        XCTAssertEqual(state.lastFedAt, now)
    }

    func testAddActivityXPZeroIsNoOp() {
        var state = PetCareState()
        XCTAssertEqual(PetCare.addActivityXP(0, state: &state, now: date("2026-01-01 10:00")), 0)
        XCTAssertEqual(state.xp, 0)
        XCTAssertNil(state.lastFedAt)
    }

    func testAddActivityXPRollsDayOver() {
        var state = PetCareState()
        _ = PetCare.addActivityXP(5, state: &state, now: date("2026-01-01 10:00"))
        XCTAssertEqual(state.dayKey, "2026-01-01")
        _ = PetCare.addActivityXP(3, state: &state, now: date("2026-01-02 09:00"))
        XCTAssertEqual(state.dayKey, "2026-01-02")
        XCTAssertEqual(state.xp, 8)
    }
}
