"use strict";var _nodeChild_process = await jitiImport("node:child_process");
...
var _childTranscript = await jitiImport("../../shared/child-transcript.ts");
var _controlChannel = await jitiImport("./control-channel.ts");
...
var _runtimeAcknowledgedExtensions = await jitiImport("../shared/runtime-acknowledged-extensions.ts");
var _chainOutputs = await jitiImport("../shared/chain-outputs.ts");
var _structuredOutput = await jitiImport("../shared/structured-output.ts");
var _processSignal = await jitiImport("../shared/process-signal.ts");
var _toolAvailability = await jitiImport("../shared/tool-availability.ts");
var _dynamicFanout = await jitiImport("../shared/dynamic-fanout.ts");
var _nestedEvents = await jitiImport("../shared/nested-events.ts");
var _modelFallback = await jitiImport("../shared/model-fallback.ts");
var _subagentStartupRetry = await jitiImport("../shared/subagent-startup-retry.ts");
...
var _processTerminal = await jitiImport("./process-terminal.ts");
var _steering = await jitiImport("./steering.ts");
...
var _sessionTokens = await jitiImport("../../shared/session-tokens.ts");
...
var _launchContract = await jitiImport("../../shared/launch-contract.ts");
var _settings = await jitiImport("../../shared/settings.ts");
var _intercomBridge = await jitiImport("../../intercom/intercom-bridge.ts");
var _acceptance = await jitiImport("../shared/acceptance.ts");
var _agentContract = await jitiImport("../shared/agent-contract.ts");
var _chainRootAttachment = await jitiImport("./chain-root-attachment.ts");
var _chainAppend = await jitiImport("./chain-append.ts");
var _turnBudget = await jitiImport("../shared/turn-budget.ts");
var _toolBudget = await jitiImport("../shared/tool-budget.ts");
var _usageBudget = await jitiImport("../shared/usage-budget.ts");
var _parallelHandoff = await jitiImport("../shared/parallel-handoff.ts");
var _settings2 = await jitiImport("../../watchdog/settings.ts");
var _childProtocol = await jitiImport("../shared/child-protocol.ts");
var _sessionLease = await jitiImport("../shared/session-lease.ts");
var _capabilityCeiling = await jitiImport("../shared/capability-ceiling.ts");
var _childStatus = await jitiImport("../../watchdog/child-status.ts");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function appendJsonl(filePath, line) {
  try {
    (0, _artifacts.appendJsonl)(filePath, line);
...
onWriterProcess)
{
  return new Promise((resolve) => {
    ...
    const processInstanceId = (0, _nodeCrypto.randomUUID)();
    onWriterProcess?.({ state: "spawning" });
    const outputStream = fs.createWriteStream(outputFile, { flags: "w" });
    const spawnEnv = { ...process.env, ...(env ?? {}), ...(0, _types.getSubagentDepthEnv)(maxSubagentDepth) };
    const spawnSpec = (0, _piSpawn.getPiSpawnCommand)(args, {
      ...(piPackageRoot ? { piPackageRoot } : {}),
      ...(piArgv1 ? { argv1: piArgv1 } : {})
    });
    const child = (0, _nodeChild_process.spawn)(spawnSpec.command, spawnSpec.args, {
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
      env: spawnEnv,
      windowsHide: true
    });
    const stderrTail = (0, _childProtocol.createBoundedByteTail)();
    const rawStdoutTail = (0, _childProtocol.createBoundedByteTail)();
    const messages = [];
    ...
    if (typeof child.pid === "number") {
      try {
        ...
        (0, _postExitStdioGuard.trySignalChild)(child, "SIGKILL");
      }
    }
    ...
    let toolCount = 0;
    const childWatchdogConfig = (0, _childStatus.decodeChildWatchdogConfig)(env?.[_childStatus.CHILD_WATCHDOG_CONFIG_ENV]);
    ...
    const appendChildLine = (type, line) => {
      ...
      if (type === "subagent.child.stdout") transcriptWriter?.writeStdoutLine(line);else
    ...
    const processStdoutLine = (line) => {
      ...
      applyChildLifecycle((0, _childProtocol.projectChildLifecycle)(event));

      if ((0, _childStatus.isChildWatchdogStatusEvent)(event)) {
        if (!childWatchdogConfig) return;
        const next = (0, _childStatus.acceptChildWatchdogEvent)({
        ...
        onChildEvent?.(event);
        if ((0, _childStatus.childWatchdogIsActive)(next)) {
          if (finalDrainTimer) {
      ...
      if (event.type === "tool_execution_start" && event.toolName) {
        toolCount += 1;
        observedMutationAttempt = observedMutationAttempt || (0, _longRunningGuard.isMutatingTool)(event.toolName, event.args);
        const toolArgs = (0, _utils.extractToolArgsPreview)(event.args ?? {});
      ...
      if ((event.type === "message_end" || event.type === "tool_result_end") && event.message) {
        ...
        const text = (0, _utils.extractTextFromContent)(event.message.content);
        ...
        if (isTerminalAssistantStop(event.message)) {
          if (!event.message.errorMessage && (0, _utils.extractTextFromContent)(event.message.content).trim()) assistantError = undefined;
          cleanTerminalAssistantStopReceived ||= !event.message.errorMessage;
          applyChildLifecycle((0, _childProtocol.projectChildLifecycle)(event, true));
        }
      }
    };
    ...
    const failProtocol = (limit) => {
      ...
      protocolError = limit;
      error = (0, _childProtocol.formatProtocolOutputLimit)(limit);
      if (!childExited) {
        (0, _postExitStdioGuard.trySignalChild)(child, "SIGTERM");
        protocolHardKillTimer = setTimeout(() => {
          if (!settled) (0, _postExitStdioGuard.trySignalChild)(child, "SIGKILL");
        }, 3000);
        protocolHardKillTimer.unref?.();
      }
    };
    const stdoutReader = (0, _childProtocol.createBoundedLineReader)({ onLine: processStdoutLine, onLimit: failProtocol });
    const stderrReader = (0, _childProtocol.createBoundedLineReader)({
      stream: "stderr",
      maxPendingLineBytes: _childProtocol.MAX_CHILD_STDERR_BYTES,
      onLine: (line) => appendChildLine("subagent.child.stderr", line),
      onLimit: (limit) => appendChildLine("subagent.child.stderr", (0, _childProtocol.formatProtocolOutputLimit)(limit))
    });
    const clearStdioGuard = (0, _postExitStdioGuard.attachPostExitStdioGuard)(child, { idleMs: 2000, hardMs: 8000 });
    ...
    registerInterrupt?.(() => {
      ...
      (0, _postExitStdioGuard.trySignalChild)(child, "SIGINT");
      setTimeout(() => {
        if (!settled && !timedOut && !stopped) (0, _postExitStdioGuard.trySignalChild)(child, "SIGTERM");
      }, 1000).unref?.();
    });
    registerTimeout?.(() => {
      if (settled || timedOut || stopped) return;
    ...
    registerTurnBudgetAbort?.((message, state) => {
      ...
      turnBudgetHardKillTimer = setTimeout(() => {
        if (!settled && !timedOut && !stopped) (0, _postExitStdioGuard.trySignalChild)(child, "SIGKILL");
      }, 4000);
    ...
    function startFinalDrain() {
      if ((0, _childStatus.childWatchdogIsActive)(childWatchdogState)) {
        armWatchdogTail();
        return;
      }
      ...
      finalDrainTimer = setTimeout(() => {
        if (settled) return;
        const termSent = (0, _postExitStdioGuard.trySignalChild)(child, "SIGTERM");
        if (!termSent) return;
        ...
        finalHardKillTimer = setTimeout(() => {
          if (settled) return;
          forcedTerminationSignal = (0, _postExitStdioGuard.trySignalChild)(child, "SIGKILL") || forcedTerminationSignal;
        }, HARD_KILL_MS);
    ...
    child.on("close", (exitCode, signal) => {
      ...
      const finalOutput = (0, _utils.getFinalOutput)(messages) || rawStdoutTail.text().trim();
      ...
      const signalError = (0, _processSignal.isUnexplainedProcessSignal)({
      ...
      }) ? (0, _processSignal.formatProcessSignalError)(signal) : undefined;
      resolve({
        stderr,
...
function resolvePiPackageRootFallback() {
  const root = (0, _piSpawn.resolveInstalledPiPackageRoot)();
  if (root) return root;
  throw new Error(`Could not resolve ${_piSpawn.PI_CODING_AGENT_PACKAGE} package root`);
}
...
async function exportSessionHtml(sessionFile, outputDir, piPackageRoot) {
  ...
  const moduleUrl = (0, _nodeUrl.pathToFileURL)(exportModulePath).href;
  const mod = await ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))(moduleUrl);
...
function createShareLink(htmlPath) {
  try {
    const auth = (0, _nodeChild_process.spawnSync)("gh", ["auth", "status"], { encoding: "utf-8" });
    if (auth.status !== 0) {
  ...
  try {
    const result = (0, _nodeChild_process.spawnSync)("gh", ["gist", "create", htmlPath], { encoding: "utf-8" });
    if (result.status !== 0) {
...

{
  if (step.importAsyncRoot) {
    ...
    ctx.registerTimeout?.(() => {
      ...
      let pid;
      try {
        pid = (0, _utils.readStatus)(step.importAsyncRoot.asyncDir)?.pid;
      } catch {
        pid = undefined;
      }
      try {
        (0, _controlChannel.deliverTimeoutRequest)({ asyncDir: step.importAsyncRoot.asyncDir, pid, source: "ancestor-timeout" });
      } catch {
    ...
    ctx.registerStop?.(() => {
      ...
      try {
        (0, _controlChannel.deliverStopRequest)({ asyncDir: step.importAsyncRoot.asyncDir, pid, source: "ancestor-stop" });
      } catch {

        // The parent runner's own stopped result is authoritative for the attached step.
      }});
    try {
      const imported = await (0, _chainRootAttachment.waitForImportedAsyncRoot)(step.importAsyncRoot, {
  ...
  (0, _structuredOutput.createStructuredOutputRuntime)(step.structuredOutputSchema, path.join(path.dirname(ctx.outputFile), "structured-output")) :
  undefined);
  const placeholderRegex = new RegExp(ctx.placeholder.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g");
  let task = step.task.replace(placeholderRegex, () => ctx.previousOutput);
  if (ctx.outputs) task = (0, _chainOutputs.resolveOutputReferences)(task, ctx.outputs);
  const taskForCompletionGuard = task;
  if (step.effectiveAcceptance) {
    const acceptancePrompt = (0, _acceptance.formatAcceptancePrompt)(step.effectiveAcceptance, { reportOptional: (0, _agentContract.isAgentContractV1)(step.agentContract) });
  ...
  if (ctx.artifactsDir && ctx.artifactConfig?.enabled !== false) {
    ...
    artifactPaths = (0, _artifacts.getArtifactPaths)(ctx.artifactsDir, ctx.id, step.agent, index);
    ...
    if (ctx.artifactConfig?.includeTranscript !== false) {
      transcriptWriter = (0, _childTranscript.createChildTranscriptWriter)({
  ...
  let turnBudget = ctx.turnBudget ? (0, _turnBudget.initialTurnBudgetState)(ctx.turnBudget) : undefined;
  let toolBudget = step.toolBudget ? (0, _toolBudget.initialToolBudgetState)(step.toolBudget) : undefined;
  ...
  modelAttemptsLoop: while (modelIndex < candidates.length) {
    ...
    ctx.onAttemptStart?.({ model: candidate, thinking: (0, _modelInfo.resolveEffectiveThinking)(candidate, step.thinking) });
    const outputSnapshot = (0, _singleOutput.captureSingleOutputSnapshot)(step.outputPath);
    ...
    const watchdogConfig = (0, _settings2.resolveWatchdogConfig)(step.cwd ?? ctx.cwd);
    const childWatchdog = watchdogConfig.ok ?
    (0, _childStatus.resolveChildWatchdogConfig)({
    ...
    undefined;
    const { args, env, tempDir, toolDiagnosticPath, runtimeAcknowledgedExtensionsPath, capabilityAudit: attemptCapabilityAudit } = (0, _piArgs.buildPiArgs)({
      ...
      systemPrompt: (0, _turnBudget.appendTurnBudgetSystemPrompt)(step.systemPrompt ?? "", ctx.turnBudget),
    ...
    if (step.definitionDigest) {
      const toolPlan = (0, _piArgs.resolvePiLaunchToolPlan)({
        tools: step.tools,
        extensions: step.extensions,
        subagentOnlyExtensions: step.subagentOnlyExtensions,
        mcpDirectTools: step.mcpDirectTools,
      ...
      launchResolvedExtensions = (0, _piArgs.projectLaunchResolvedChildExtensions)(toolPlan);
      actualLaunchContractDigest = (0, _launchContract.launchBindingDigest)({
        definitionDigest: step.definitionDigest,
        task: step.launchBindingTask ?? task,
        ...(candidate ? { model: candidate } : {}),
        modelCandidates: candidates,
        ...((0, _modelInfo.resolveEffectiveThinking)(candidate, step.thinking) ? { thinking: (0, _modelInfo.resolveEffectiveThinking)(candidate, step.thinking) } : {}),
        systemPrompt: (0, _turnBudget.appendTurnBudgetSystemPrompt)(step.systemPrompt ?? "", ctx.turnBudget),
        systemPromptMode: step.systemPromptMode,
        inheritProjectContext: step.inheritProjectContext,
        inheritSkills: step.inheritSkills,
    ...
    if (run.turnBudget) turnBudget = run.turnBudget;else
    if (ctx.turnBudget) {
      ...
      if (turnCount > 0 && turnCount < ctx.turnBudget.maxTurns) {
        ...
        const decision = (0, _turnBudget.turnBudgetDecision)(
          ctx.turnBudget,
          turnCount,
        ...
        (0, _turnBudget.turnBudgetDeferredState)(ctx.turnBudget, turnCount) :
        (0, _turnBudget.turnBudgetState)(ctx.turnBudget, turnCount, decision === "abort");
      }
    }
    const toolAvailabilityError = run.exitCode === 0 && !run.error ?
    (0, _toolAvailability.readChildToolDiagnosticError)(toolDiagnosticPath) :
    undefined;
    const runtimeAcknowledgedExtensions = (0, _runtimeAcknowledgedExtensions.readRuntimeAcknowledgedExtensions)(runtimeAcknowledgedExtensionsPath);
    (0, _piArgs.cleanupTempDir)(tempDir);

    const hiddenError = run.exitCode === 0 && !run.error && !toolAvailabilityError ? (0, _utils.detectSubagentError)(run.messages) : null;
    ...
    !hiddenError?.hasError || (0, _utils.hasEmptyTerminalAssistantResponse)(run.messages)) ?
    ...
    if (effectiveStructuredOutput && run.exitCode === 0 && !run.error && !toolAvailabilityError && !hiddenError?.hasError && !emptyOutputError) {
      const structured = await (0, _structuredOutput.readStructuredOutput)({
        schema: effectiveStructuredOutput.schema,
        schemaPath: effectiveStructuredOutput.schemaPath,
        outputPath: effectiveStructuredOutput.outputPath
      });
      if (structured.error) structuredError = structured.error;else
      structuredOutput = structured.value;
    }
    const completionGuardEnabled = (0, _agentContract.isAgentContractV1)(step.agentContract) ? step.completionGuard === true : step.completionGuard !== false;
    ...
    (0, _completionGuard.evaluateCompletionMutationGuard)({
      agent: step.agent,
    ...
    undefined;
    const completionGuardError = completionGuardTriggered && !(0, _agentContract.isAgentContractV1)(step.agentContract) ?
    "Subagent completed without making edits for an implementation task.\nIt appears to have returned planning or scratchpad output instead of applying changes." :
    undefined;
    const effectiveExitCode = toolAvailabilityError || completionGuardTriggered && !(0, _agentContract.isAgentContractV1)(step.agentContract) || structuredError || emptyOutputError ?
    1 :
    ...
    1 :
    run.exitCode;
    const signalError = run.exitCode !== 0 && (0, _processSignal.isUnexplainedProcessSignal)({
      processSignal: run.processSignal,
      interrupted: run.interrupted,
      timedOut: run.timedOut,
      stopped: run.stopped,
      turnBudgetExceeded: run.turnBudgetExceeded
    }) ? (0, _processSignal.formatProcessSignalError)(run.processSignal) : undefined;
    ...
    if (step.toolBudget) {
      ...
      const blockedMessage = toolMessages.find((message) => (0, _utils.extractTextFromContent)(message.content).includes("Tool budget hard limit reached"));
      toolBudgetBlocked = Boolean(blockedMessage);
      toolBudget = (0, _toolBudget.toolBudgetState)(step.toolBudget, toolMessages.length, blockedMessage ? blockedMessage.toolName : undefined);
    }
    finalResult = { ...run, exitCode: effectiveExitCode, model: candidate ?? run.model, error, structuredOutput, runtimeAcknowledgedExtensions, ...(step.agentContract ? { agentContract: step.agentContract } : {}), ...(fileMutationEffect ? { effects: { fileMutation: fileMutationEffect } } : {}) };
    ...
    const startupFailure = (0, _subagentStartupRetry.isRetryableSubagentStartupFailure)({
      exitCode: effectiveExitCode,
      error,
    ...
    const retryDelayMs = _subagentStartupRetry.SUBAGENT_STARTUP_RETRY_DELAYS_MS[startupAttemptIndex];
    if (startupFailure && retryDelayMs !== undefined) {
      const retryNote = (0, _subagentStartupRetry.formatSubagentStartupRetryNote)({
        model: attempt.model,
        attempt: startupAttemptIndex + 1,
        maxAttempts: _subagentStartupRetry.SUBAGENT_STARTUP_RETRY_DELAYS_MS.length + 1,
        delayMs: retryDelayMs
      });
      const shouldRetry = await (0, _subagentStartupRetry.waitForSubagentStartupRetry)(retryDelayMs, [ctx.timeoutSignal, ctx.stopSignal]);
    ...
    if (startupFailure) {
      const startupError = (0, _subagentStartupRetry.formatSubagentStartupRetryExhaustedError)({
        model: attempt.model,
    ...
    if (!(0, _modelFallback.isRetryableModelFailure)(error) || modelIndex === candidates.length - 1) break modelAttemptsLoop;
    attemptNotes.push((0, _modelFallback.formatModelAttemptNote)(attempt, candidates[modelIndex + 1]));
    modelIndex += 1;
    startupAttemptIndex = 0;
  }

  const rawOutput = finalResult?.finalOutput ?? "";
  const outputForPersistence = (0, _acceptance.stripAcceptanceReport)(rawOutput);
  const resolvedOutput = step.outputPath && finalResult?.exitCode === 0 ?
  (0, _singleOutput.resolveSingleOutput)(step.outputPath, outputForPersistence, finalOutputSnapshot) :
  { fullOutput: outputForPersistence };
  const output = (0, _acceptance.stripAcceptanceReport)(resolvedOutput.fullOutput);
  const outputReference = resolvedOutput.savedPath ? (0, _singleOutput.formatSavedOutputReference)(resolvedOutput.savedPath, output) : undefined;
  ...
  if (finalResult?.stopped && !outputForSummary.trim()) {
    ...
    outputForSummary = (0, _turnBudget.formatTurnBudgetOutput)((0, _turnBudget.turnBudgetExceededMessage)(turnBudget, turnBudget.turnCount), outputForSummary);
  } else if (!finalResult?.timedOut && !finalResult?.stopped && turnBudget?.outcome === "termination-deferred") {
    const note = (0, _turnBudget.turnBudgetDeferredNote)(turnBudget, turnBudget.terminationDeferredAtTurn ?? turnBudget.turnCount);
    ...
    const note = (0, _turnBudget.turnBudgetSoftNote)(turnBudget, turnBudget.wrapUpRequestedAtTurn ?? turnBudget.turnCount);
    outputForSummary = outputForSummary.trim() ? `${note}\n\n${outputForSummary}` : note;
  }
  const outputForAcceptance = rawOutput;
  const childWrittenOutput = step.outputPath ?
  (0, _singleOutput.extractChildWrittenOutput)(finalResult?.messages, step.outputPath, step.cwd ?? ctx.cwd) :
  undefined;
  const outputState = finalResult?.outputState === "present" ||
  ...
  await (0, _acceptance.evaluateAcceptance)({
    ...
    reportOptional: (0, _agentContract.isAgentContractV1)(step.agentContract)
  }) :
  undefined;
  ...
  (0, _acceptance.buildSkippedAcceptanceLedger)(step.effectiveAcceptance, { id: "stopped", message: "Acceptance was not evaluated because the subagent was stopped." }) :
  timedOutAfterAcceptance ?
  (0, _acceptance.buildSkippedAcceptanceLedger)(step.effectiveAcceptance, { id: "timeout", message: "Acceptance was not evaluated because the subagent timed out." }) :
  turnBudgetExceeded ?
  (0, _acceptance.buildSkippedAcceptanceLedger)(step.effectiveAcceptance, { id: "turn-budget", message: "Acceptance was not evaluated because the subagent exceeded its turn budget." }) :
  acceptance :
  undefined;
  const acceptanceFailure = effectiveAcceptance ? (0, _acceptance.acceptanceFailureMessage)(effectiveAcceptance) : undefined;
  const acceptanceCanFailRun = acceptanceFailure && effectiveAcceptance?.explicit && (finalResult?.exitCode ?? 1) === 0 && !finalResult?.interrupted && !timedOutAfterAcceptance && !stoppedAfterAcceptance && !turnBudgetExceeded && !(0, _agentContract.isAgentContractV1)(step.agentContract);
  ...
  turnBudgetExceeded ?
  finalResult?.error ?? (turnBudget ? (0, _turnBudget.turnBudgetExceededMessage)(turnBudget, turnBudget.turnCount) : "Subagent exceeded turn budget.") :
  ...
  if (artifactPaths && ctx.artifactConfig?.enabled !== false) {
    if (ctx.artifactConfig?.includeOutput !== false) {
      fs.writeFileSync(artifactPaths.outputPath, (0, _artifacts.formatOutputArtifactContent)({
        output,
    ...
    if (ctx.artifactConfig?.includeMetadata !== false) {
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          ...(finalResult?.runtimeAcknowledgedExtensions ? { runtimeAcknowledgedExtensions: finalResult.runtimeAcknowledgedExtensions } : {}),
  ...
  const result = {
    ...
    ...(finalResult?.effects ? { effects: finalResult.effects } : {}),
  ...
  return (0, _agentContract.isAgentContractV1)(step.agentContract) ? (0, _agentContract.attachContractProjections)(result) : result;
}
...
function appendCapabilityCeilingAppliedEvent(eventsPath, runId, stepIndex, agent, result) {
...

{
  ...
  (0, _atomicJson.writeAtomicJson)(input.statusPath, input.statusPayload);
...
group)
{
  ...
  const diffs = (0, _worktree.diffWorktrees)(worktreeSetup, group.parallel.map((task) => task.agent), diffsDir);
  return { diffs, summary: (0, _worktree.formatWorktreeDiffSummary)(diffs) };
}
...
function ensureParallelProgressFile(cwd, group) {
  ...
  (0, _settings.writeInitialProgressFile)(cwd);
}
...

{
  ...
  return (0, _artifacts.getArtifactPaths)(
    input.artifactsDir,
    input.runId,
    input.agent,
...
onWriterProcess)
{
  ...
  config;
  const globalSemaphore = new _parallelUtils.Semaphore(config.globalConcurrencyLimit ?? _parallelUtils.DEFAULT_GLOBAL_CONCURRENCY_LIMIT);
  let previousOutput = "";
  const outputs = {};
  const results = [];
  ...
  const controlConfig = config.controlConfig ?? _subagentControl.DEFAULT_CONTROL_CONFIG;
  ...
  let pendingCheckpointDecision;
  let wakeCheckpointDecision;
  ...
  const flatSteps = (0, _parallelUtils.flattenSteps)(steps);
  ...
  for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
    ...
    if ((0, _parallelUtils.isCheckpointRunnerStep)(step)) {
    ...
    if ((0, _parallelUtils.isParallelGroup)(step)) {
      ...
      for (const task of step.parallel) {
        ...
        initialStatusSteps.push({
          agent: task.agent,
          ...((0, _chainAppend.statusStepDescription)(task.task) ? { description: (0, _chainAppend.statusStepDescription)(task.task) } : {}),
          ...
          status: "pending",
          ...(task.toolBudget ? { toolBudget: (0, _toolBudget.initialToolBudgetState)(task.toolBudget) } : {}),
    ...
    } else if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
      ...
      initialStatusSteps.push({
        ...
        status: "pending",
        ...(step.parallel.toolBudget ? { toolBudget: (0, _toolBudget.initialToolBudgetState)(step.parallel.toolBudget) } : {}),
        recentTools: [],
        recentOutput: []
      });
      flatStepCount++;
      ...
      initialStatusSteps.push({
        agent: step.agent,
        ...((0, _chainAppend.statusStepDescription)(step.task) ? { description: (0, _chainAppend.statusStepDescription)(step.task) } : {}),
        ...
        ...(step.capabilityCeiling ? { capabilityCeiling: step.capabilityCeiling } : {}),
        status: "pending",
        ...(step.toolBudget ? { toolBudget: (0, _toolBudget.initialToolBudgetState)(step.toolBudget) } : {}),
  ...
  const statusPayload = {
    lifecycleArtifactVersion: _types.SUBAGENT_LIFECYCLE_ARTIFACT_VERSION,
    runId: id,
    ...(config.sessionId ? { sessionId: config.sessionId } : {}),
    mode: config.resultMode ?? (flatSteps.length > 1 ? "chain" : "single"),
    ...(config.nestedRoute ? { isNested: true } : {}),
    state: "running",
    steering: (0, _steering.createSteeringStatus)(),
    lastActivityAt: overallStartTime,
    startedAt: overallStartTime,
    lastUpdate: overallStartTime,
    ...(config.timeoutMs !== undefined ? { timeoutMs: config.timeoutMs } : {}),
    ...(config.deadlineAt !== undefined ? { deadlineAt: config.deadlineAt } : {}),
    ...(config.turnBudget ? { turnBudget: (0, _turnBudget.initialTurnBudgetState)(config.turnBudget) } : {}),
    ...(config.toolBudget ? { toolBudget: (0, _toolBudget.initialToolBudgetState)(config.toolBudget) } : {}),
    ...(config.usageBudget ? { usageBudget: (0, _usageBudget.usageBudgetState)(config.usageBudget, undefined) } : {}),
    pid: process.pid,
    cwd,
    currentStep: 0,
    ...
    ...(config.runnerProcessInstanceId ? { processTerminal: { version: 1, state: "pending", runId: id, runnerProcessInstanceId: config.runnerProcessInstanceId } } : {}),
  ...
  (0, _atomicJson.writeAtomicJson)(statusPath, statusPayload);
  let pendingParallelUsageCost = { inputTokens: 0, outputTokens: 0, costUsd: 0 };
  const currentUsageTotals = () => {
  ...
  const refreshUsageBudget = () => {
    statusPayload.usageBudget = (0, _usageBudget.usageBudgetState)(config.usageBudget, currentUsageTotals());
    return statusPayload.usageBudget;
  };
  const emitNestedSelfEvent = (type) => {
    if (!config.nestedRoute || !config.nestedSelf) return;
    try {
      (0, _nestedEvents.writeNestedEvent)(config.nestedRoute, {
        type,
        ts: Date.now(),
        parentRunId: config.nestedSelf.parentRunId,
        parentStepIndex: config.nestedSelf.parentStepIndex,
        child: (0, _nestedEvents.nestedSummaryFromAsyncStatus)(statusPayload, asyncDir, {
          id,
          parentRunId: config.nestedSelf.parentRunId,
          parentStepIndex: config.nestedSelf.parentStepIndex,
  ...
  const refreshWorkflowGraph = () => {
    ...
    const updateNode = (node) => {
      ...
      if (node.children?.length) {
        if (node.children.every((child) => child.status === "completed")) node.status = "completed";else
        if (node.children.some((child) => child.status === "running")) node.status = "running";else
        if (node.children.some((child) => child.status === "stopped")) node.status = "stopped";else
        if (node.children.some((child) => child.status === "rejected")) node.status = "rejected";else
        if (node.children.some((child) => child.status === "failed")) node.status = "failed";else
        if (node.children.some((child) => child.status === "paused")) node.status = "paused";
      }
  ...
  const interruptNestedAsyncDescendants = () => {
    ...
    let registry;
    try {
      registry = (0, _nestedEvents.projectNestedEvents)(config.nestedRoute);
    } catch (error) {
      appendJsonl(eventsPath, JSON.stringify({
    ...
    for (const run of nestedRuns(registry.children)) {
      ...
      const nestedAsyncDir = run.asyncDir ?? (0, _nestedEvents.resolveNestedAsyncDir)(config.nestedRoute.rootRunId, run);
      if (!nestedAsyncDir) continue;
      try {
        (0, _controlChannel.deliverInterruptRequest)({ asyncDir: nestedAsyncDir, pid: run.pid, source: "ancestor-interrupt" });
      } catch (error) {
        appendJsonl(eventsPath, JSON.stringify({
          type: "subagent.nested.interrupt_failed",
          ts: Date.now(),
  ...
  const stopNestedAsyncDescendants = () => {
    ...
    for (const run of nestedRuns(registry.children)) {
      ...
      if (!nestedAsyncDir) continue;
      try {
        (0, _controlChannel.deliverStopRequest)({ asyncDir: nestedAsyncDir, pid: run.pid, source: "ancestor-stop" });
      } catch (error) {
        appendJsonl(eventsPath, JSON.stringify({
          type: "subagent.nested.stop_failed",
          ts: Date.now(),
  ...
  const timeoutNestedAsyncDescendants = () => {
    ...
    for (const run of nestedRuns(registry.children)) {
      ...
      if (!nestedAsyncDir) continue;
      try {
        (0, _controlChannel.deliverTimeoutRequest)({ asyncDir: nestedAsyncDir, pid: run.pid, source: "ancestor-timeout" });
      } catch (error) {
        appendJsonl(eventsPath, JSON.stringify({
          type: "subagent.nested.timeout_failed",
          ts: Date.now(),
  ...
  const pausedStepResult = (agent, context) => ({
    agent,
    context,
  ...
  const timedOutStepResult = (agent, context) => ({
    agent,
    context,
  ...
  const stoppedStepResult = (agent, context) => ({
    agent,
    context,
    output: stopMessage,
    error: stopMessage,
  ...
  const consumePendingAppendRequests = () => {
    ...
    const requests = (0, _chainAppend.consumeChainAppendRequests)(asyncDir);
    if (requests.length === 0) {
      const pendingAppends = (0, _chainAppend.countPendingChainAppendRequests)(asyncDir);
    ...
    const now = Date.now();
    const pendingAppends = (0, _chainAppend.countPendingChainAppendRequests)(asyncDir);
    const added = (0, _chainAppend.appendRunnerStepsToStatus)({
      status: statusPayload,
      steps: appendedSteps,
      now,
      pendingAppends
    });
    mutatingFailureStates.push(...Array.from({ length: added.addedFlatSteps }, () => (0, _longRunningGuard.createMutatingFailureState)()));
    pendingToolResults.push(...Array.from({ length: added.addedFlatSteps }, () => undefined));
    if (config.childIntercomTargets) {
      config.childIntercomTargets = statusPayload.steps.map((statusStep, index) => (0, _intercomBridge.resolveSubagentIntercomTarget)(id, statusStep.agent, index));
    }
    writeStatusPayload();
  ...
  const mutatingFailureStates = initialStatusSteps.map(() => (0, _longRunningGuard.createMutatingFailureState)());
  const pendingToolResults = initialStatusSteps.map(() => undefined);
  const supervisorAttentionSteps = new Map();
  ...
  const appendControlEvent = (event) => {
    ...
    if (channels.length === 0 || !(0, _subagentControl.claimControlNotification)(controlConfig, event, emittedControlEventKeys, childIntercomTarget)) return;
    appendJsonl(eventsPath, JSON.stringify({
      type: "subagent.control",
      event,
      channels,
      childIntercomTarget,
      noticeText: (0, _subagentControl.formatControlNoticeMessage)(event, childIntercomTarget),
      ...(config.controlIntercomTarget && channels.includes("intercom") ? {
        intercom: {
          to: config.controlIntercomTarget,
          message: (0, _subagentControl.formatControlIntercomMessage)(event, childIntercomTarget)
        }
      } : {})
    }));
  };
  ...
  const syncAggregateActivityState = () => {
  ...
  const maybeEmitActiveLongRunning = (flatIndex, now) => {
    ...
    const reason = (0, _longRunningGuard.nextLongRunningTrigger)(controlConfig, {
  ...
  const recordSteeringLifecycle = (request, targets) => {
    const lifecycle = (0, _steering.steeringStatus)(statusPayload);
    (0, _steering.recordSteeringRequest)(lifecycle, { id: request.id, requestedAt: request.ts, source: request.source, message: request.message, targets });
    for (const target of targets) {
      const step = statusPayload.steps[target.index];
      if (!step) continue;
      step.steering ??= (0, _steering.createSteeringStatus)();
      (0, _steering.recordSteeringRequest)(step.steering, { id: request.id, requestedAt: request.ts, source: request.source, message: request.message, targets: [target] });
    }
  };
  ...
  fields = {}) =>
  {
    const updated = (0, _steering.updateSteeringTarget)((0, _steering.steeringStatus)(statusPayload), requestId, index, state, now, fields);
    const step = statusPayload.steps[index];
    if (step?.steering) (0, _steering.updateSteeringTarget)(step.steering, requestId, index, state, now, fields);
    return updated;
  };
  const emitTerminalSteeringNotice = (requestId, failureMessage) => {
    const state = (0, _steering.terminalSteeringNoticeState)((0, _steering.steeringStatus)(statusPayload), requestId);
    if (state === "partial") emitSteeringNotice(requestId, "partial", `Steering partially delivered for run ${id}.`);else
    if (state === "failed") emitSteeringNotice(requestId, "failed", failureMessage);
  };
  const deliverSteerRequest = (request) => {
    ...
    const targetStates = targets.map((index) => {
      const step = statusPayload.steps[index];
      if (!step) return { index, state: "failed", reason: "child index out of range" };
      if (step.status === "pending") return { index, state: "scheduled" };
      if (step.status !== "running") return { index, state: "failed", reason: `child is ${step.status}` };
      if (steeringCapabilities.get(index)?.supported === false) return { index, state: "failed", reason: "child Pi session does not support steering" };
    ...
    for (const target of targetStates) {
      if (target.state === "routed") {
        try {
          (0, _controlChannel.enqueueStepSteer)(asyncDir, target.index, request);
  ...
  const flushPendingStepSteers = (flatIndex) => {
    ...
    for (const request of pendingStepSteers.splice(0)) {
      if (request.targetIndex === undefined) deliverSteerRequest({ ...request, targetIndex: flatIndex });else
      if (request.targetIndex === flatIndex) deliverSteerRequest(request);else
  ...
  toolWorkActiveOrStarting) =>
  {
    ...
    if (!step.wrapUpRequested) {
      ...
      appendRecentStepOutput(step, [(0, _turnBudget.turnBudgetSoftNote)(budget, turnCount)]);
    }
    const decision = (0, _turnBudget.turnBudgetDecision)(budget, turnCount, terminalAssistantStop, toolWorkActiveOrStarting);
    if (decision === "defer") {
      const firstDeferredTurn = step.turnBudget?.terminationDeferredAtTurn;
      const deferredState = (0, _turnBudget.turnBudgetDeferredState)(budget, turnCount, firstDeferredTurn);
    ...
    const state = (0, _turnBudget.turnBudgetState)(budget, turnCount, false);
    step.turnBudget = state;
    statusPayload.turnBudget = state;
    if (decision !== "abort") return;
    const exceededState = (0, _turnBudget.turnBudgetState)(budget, turnCount, true);
    const message = (0, _turnBudget.turnBudgetExceededMessage)(budget, turnCount);
  ...
  const updateStepFromChildEvent = (flatIndex, event) => {
    ...
    if (event.type === "tool_execution_start" && event.toolName) {
      const mutates = (0, _longRunningGuard.isMutatingTool)(event.toolName, event.args);
      const currentPath = (0, _longRunningGuard.resolveCurrentPath)(event.toolName, event.args);
      step.toolCount = (step.toolCount ?? 0) + 1;
      const configuredToolBudget = flatSteps[flatIndex]?.toolBudget;
      if (configuredToolBudget) {
        step.toolBudget = (0, _toolBudget.toolBudgetState)(configuredToolBudget, step.toolCount);
        statusPayload.toolBudget = step.toolBudget;
      }
      step.currentTool = event.toolName;
      step.currentToolArgs = (0, _utils.extractToolArgsPreview)(event.args ?? {});
      ...
      if (controlConfig.enabled && isBlockingSupervisorTool(event.toolName, event.args) && step.activityState !== "needs_attention") {
        ...
        statusPayload.activityState = "needs_attention";
        appendControlEvent((0, _subagentControl.buildControlEvent)({
      ...
      const resultText = (0, _utils.extractTextFromContent)(event.message.content);
      if (toolSnapshot && resultText.includes("Tool budget hard limit reached")) {
        const configuredToolBudget = flatSteps[flatIndex]?.toolBudget;
        if (configuredToolBudget) {
          step.toolBudget = (0, _toolBudget.toolBudgetState)(configuredToolBudget, step.toolCount ?? 0, toolSnapshot.tool);
      ...
      if (toolSnapshot?.mutates && (0, _longRunningGuard.didMutatingToolFail)(resultText)) {
        ...
        (0, _longRunningGuard.recordMutatingFailure)(state, {
          tool: toolSnapshot.tool,
          path: toolSnapshot.path,
          error: resultText.split("\n").find((line) => line.trim())?.trim().slice(0, 180) ?? "mutating tool failed",
          ts: now
        }, mutatingFailureWindowMs);
        if (controlConfig.enabled && (0, _longRunningGuard.shouldEscalateMutatingFailures)(state, controlConfig.failedToolAttemptsBeforeAttention) && step.activityState !== "needs_attention") {
          const previous = step.activityState;
          step.activityState = "needs_attention";
          statusPayload.activityState = "needs_attention";
          appendControlEvent((0, _subagentControl.buildControlEvent)({
            ...
            recentFailureSummary: (0, _longRunningGuard.summarizeRecentMutatingFailures)(state)
          }));
        }
      } else if (toolSnapshot?.mutates) {
        (0, _longRunningGuard.resetMutatingFailureState)(mutatingFailureStates[flatIndex]);
      }
    } else if (event.type === "message_end" && event.message?.role === "assistant") {
      appendRecentStepOutput(step, (0, _acceptance.stripAcceptanceReport)((0, _utils.extractTextFromContent)(event.message.content)).split("\n").slice(-10));
  ...
  const interruptRunner = () => {
    (0, _controlChannel.consumeInterruptRequest)(asyncDir);
  ...
  const disposeControlInbox = (0, _controlChannel.watchAsyncControlInbox)(asyncDir, {
  ...
  while (true) {
    ...
    if (statusPayload.usageBudget?.exhausted) {
      ...
      statusPayload.state = "failed";
      statusPayload.error = (0, _usageBudget.usageBudgetExceededMessage)(statusPayload.usageBudget);
      statusPayload.currentStep = flatIndex;
      statusPayload.lastUpdate = Date.now();
      writeStatusPayload();
      break;
    ...
    if ((0, _parallelUtils.isCheckpointRunnerStep)(step)) {
      ...
      const checkpoint = { name: step.checkpoint, ...(step.message ? { message: step.message } : {}), status: "pending", stepIndex };
      statusPayload.state = "paused";
      statusPayload.currentStep = flatIndex;
      ...
      if (decision === "approved") {
        ...
        const approved = { ...checkpoint, status: "approved", approvedAt };
      ...
      if (decision === "rejected") {
        ...
        const rejected = { ...checkpoint, status: "rejected", rejectedAt };
    ...
    if ((0, _parallelUtils.isDynamicRunnerGroup)(step)) {
      const groupStartFlatIndex = flatIndex;
      let materialized;
      try {
        materialized = (0, _dynamicFanout.materializeDynamicParallelStep)(step, outputs, stepIndex, { maxItems: config.dynamicFanoutMaxItems, allowRunnerFields: true });
        if (materialized.parallel.length > 1 && step.parallel.outputPath && !step.parallel.namespaceOutputPath) {
          throw new _dynamicFanout.DynamicFanoutError(`Dynamic chain step ${stepIndex + 1} materialized ${materialized.parallel.length} items that resolve output to the same path: ${step.parallel.outputPath}. Remove the explicit output path or use an inherited relative agent output so each item can be isolated.`);
        }
        if (materialized.collectedOnEmpty) await (0, _dynamicFanout.validateDynamicCollection)(step.collect.outputSchema, materialized.collectedOnEmpty);
      } catch (error) {
        const now = Date.now();
        const message = error instanceof _dynamicFanout.DynamicFanoutError ? error.message : error instanceof Error ? error.message : String(error);
        statusPayload.state = "failed";
        statusPayload.error = message;
        statusPayload.currentStep = flatIndex;
      ...
      const effectiveDynamicGroupAcceptance = (0, _acceptance.resolveEffectiveAcceptance)({
      ...
      if (materialized.parallel.length === 0) {
        ...
        await (0, _acceptance.evaluateAcceptance)({
          ...
          output: "",
          report: (0, _acceptance.aggregateAcceptanceReport)({
            results: [],
        ...
        const groupAcceptanceFailure = effectiveGroupAcceptance && (!(0, _agentContract.isAgentContractV1)(step.agentContract) || step.gateOn === "acceptance") ? (0, _acceptance.acceptanceFailureMessage)(effectiveGroupAcceptance) : undefined;
      ...
      const dynamicSteps = materialized.parallel.map((task, itemIndex) => {
        ...
        const model = thinkingOverride ? (0, _piArgs.applyThinkingSuffix)(step.parallel.model, thinkingOverride, true) : step.parallel.model;
        const thinking = thinkingOverride ? (0, _modelInfo.resolveEffectiveThinking)(model, thinkingOverride) : undefined;
        ...
        const materializedTask = step.parallel.namespaceOutputPath ? (0, _singleOutput.injectSingleOutputInstruction)(taskText, outputPath, step.parallel) : taskText;
        return {
          ...step.parallel,
          task: materializedTask,
          effectiveAcceptance: (0, _acceptance.resolveEffectiveAcceptance)({
          ...
          systemPrompt: step.parallel.namespaceOutputPath ? (0, _singleOutput.injectOutputPathSystemPrompt)(step.parallel.systemPrompt ?? "", outputPath, step.parallel) : step.parallel.systemPrompt,
          outputPath,
          ...
          ...(thinkingOverride ? {
            ...(model ? { model } : {}),
            ...(thinking ? { thinking } : {}),
            ...(step.parallel.modelCandidates ? { modelCandidates: step.parallel.modelCandidates.map((candidate) => (0, _piArgs.applyThinkingSuffix)(candidate, thinkingOverride, true)) } : {})
          } : {}),
      ...
      mutatingFailureStates.splice(groupStartFlatIndex, 1, ...dynamicStatusSteps.map(() => (0, _longRunningGuard.createMutatingFailureState)()));
      ...
      const concurrency = step.concurrency ?? _parallelUtils.MAX_PARALLEL_CONCURRENCY;
      const failFast = step.failFast ?? false;
      let aborted = false;
      const parallelResults = await (0, _parallelUtils.mapConcurrent)(dynamicSteps, concurrency, async (task, taskIdx) => {
        const fi = groupStartFlatIndex + taskIdx;
        refreshUsageBudget();
        if (statusPayload.usageBudget?.exhausted) {
          const skippedAt = Date.now();
          const message = (0, _usageBudget.usageBudgetExceededMessage)(statusPayload.usageBudget);
        ...
        const singleResult = await runSingleStep(task, {
          ...
          steerInboxDir: (0, _controlChannel.stepSteerInboxDir)(asyncDir, fi),
          steerCapabilityPath: (0, _controlChannel.steerCapabilityPath)(asyncDir, fi),
          steerAckDir: (0, _controlChannel.steerAcksDir)(asyncDir, fi),
        ...
        statusPayload.steps[fi].thinking = (0, _modelInfo.resolveEffectiveThinking)(singleResult.model, statusPayload.steps[fi].thinking);
      ...
      refreshUsageBudget();
      const collection = (0, _dynamicFanout.collectDynamicResults)(step, materialized.items, parallelResults);
      ...
      filter(({ result, task }) => (0, _agentContract.isAgentContractV1)(task?.agentContract ?? step.agentContract) && task?.gateOn === "acceptance" && result.acceptance?.status === "rejected");
      if (acceptanceFailures.length > 0) {
        const message = acceptanceFailures.
        map(({ result, originalIndex }) => `Dynamic item ${originalIndex + 1} (${result.agent}, key ${materialized.items[originalIndex]?.key ?? originalIndex}) acceptance rejected: ${(0, _acceptance.acceptanceFailureMessage)(result.acceptance) ?? "acceptance rejected"}`).
        join("\n");
      ...
      if (failures.length === 0 && acceptanceFailures.length === 0) {
        try {
          await (0, _dynamicFanout.validateDynamicCollection)(step.collect.outputSchema, collection);
          outputs[step.collect.as] = {
            text: JSON.stringify(collection),
            structured: collection,
            agent: step.parallel.agent,
          ...
          const groupTimedOut = !groupStopped && (timedOut || timeoutAbortController.signal.aborted);
          const effectiveGroupAcceptance = groupTimedOut || groupStopped ? undefined : groupAcceptance;
          const groupAcceptanceFailure = effectiveDynamicGroupAcceptance.explicit && effectiveGroupAcceptance && (!(0, _agentContract.isAgentContractV1)(step.agentContract) || step.gateOn === "acceptance") ? (0, _acceptance.acceptanceFailureMessage)(effectiveGroupAcceptance) : undefined;
      ...
      previousOutput = (0, _parallelUtils.aggregateParallelOutputs)(
    ...
    if ((0, _parallelUtils.isParallelGroup)(step)) {
      const group = step;
      const concurrency = group.concurrency ?? _parallelUtils.MAX_PARALLEL_CONCURRENCY;
      ...
      if (group.worktree) {
        const worktreeTaskCwdConflict = (0, _worktree.findWorktreeTaskCwdConflict)(group.parallel, cwd);
        if (worktreeTaskCwdConflict) {
          ...
          markParallelGroupSetupFailure({
            ...
            group,
            groupStartFlatIndex,
            setupError: (0, _worktree.formatWorktreeTaskCwdConflict)(worktreeTaskCwdConflict, cwd),
            failedAt,
            statusPath,
            eventsPath,
            asyncDir,
        ...
        try {
          worktreeSetup = (0, _worktree.createWorktrees)(cwd, `${id}-s${stepIndex}`, group.parallel.length, {
      ...
      try {
        ...
        const parallelResults = await (0, _parallelUtils.mapConcurrent)(
          group.parallel,
          concurrency,
          async (task, taskIdx) => {
            const fi = groupStartFlatIndex + taskIdx;
        ...
        for (let t = 0; t < group.parallel.length; t++) {
          ...
          (0, _sessionTokens.parseSessionTokens)(path.join(config.sessionDir, `parallel-${t}`)) :
          null;
        ...
        for (let t = 0; t < group.parallel.length; t++) {
          ...
          if (outputName) outputs[outputName] = (0, _chainOutputs.outputEntryFromAsyncResult)({
        ...
        if (worktreeSetup) {
          ...
          const cleanup = (0, _worktree.cleanupWorktrees)(worktreeSetup);
          try {
            statusPayload.parallelHandoff = (0, _parallelHandoff.writeParallelHandoffGroup)({
              manifestPath: (0, _parallelHandoff.parallelHandoffPath)(asyncDir),
              runId: id,
              ...
              results: parallelResults.map((result) => ({
                agent: result.agent,
                status: result.stopped || result.exitCode !== 0 && (0, _processSignal.isUnexplainedProcessSignal)({
            ...
            previousOutput = `${previousOutput}\n\n${(0, _parallelHandoff.formatParallelHandoffReference)(statusPayload.parallelHandoff)}`;
          } catch (error) {
            previousOutput = `${previousOutput}\n\n${(0, _parallelHandoff.formatParallelHandoffError)(error)}`;
          }
          writeStatusPayload();
        }
        ...
        appendJsonl(eventsPath, JSON.stringify({
          ...
          parallelResults.every((result, index) => !((0, _agentContract.isAgentContractV1)(group.parallel[index]?.agentContract) && group.parallel[index]?.gateOn === "acceptance" && result.acceptance?.status === "rejected"))
        }));

        const acceptanceGateFailure = parallelResults.
        map((result, index) => ({ result, index, task: group.parallel[index] })).
        find(({ result, task }) => (0, _agentContract.isAgentContractV1)(task?.agentContract) && task?.gateOn === "acceptance" && result.acceptance?.status === "rejected");
        if (acceptanceGateFailure) {
          statusPayload.error = (0, _acceptance.acceptanceFailureMessage)(acceptanceGateFailure.result.acceptance) ?? "Parallel acceptance gate rejected the step.";
          writeStatusPayload();
          break;
        }
        ...
        if (worktreeSetup && !worktreeFinalized) (0, _worktree.cleanupWorktrees)(worktreeSetup);
      }
    } else {
      const seqStep = step;
      ...
      const singleResult = await runSingleStep(seqStep, {
        ...
        steerInboxDir: (0, _controlChannel.stepSteerInboxDir)(asyncDir, flatIndex),
        steerCapabilityPath: (0, _controlChannel.steerCapabilityPath)(asyncDir, flatIndex),
        steerAckDir: (0, _controlChannel.steerAcksDir)(asyncDir, flatIndex),
        piPackageRoot: config.piPackageRoot,
        piArgv1: config.piArgv1,
      ...
      if (seqStep.outputName) {
        outputs[seqStep.outputName] = (0, _chainOutputs.outputEntryFromAsyncResult)({
          agent: singleResult.agent,
      ...
      const cumulativeTokens = config.sessionDir ? (0, _sessionTokens.parseSessionTokens)(config.sessionDir) : null;
      ...
      statusPayload.steps[flatIndex].thinking = (0, _modelInfo.resolveEffectiveThinking)(singleResult.model, statusPayload.steps[flatIndex].thinking);
      ...
      flatIndex++;
      if ((0, _agentContract.isAgentContractV1)(seqStep.agentContract) && seqStep.gateOn === "acceptance" && singleResult.acceptance?.status === "rejected") {
        statusPayload.error = (0, _acceptance.acceptanceFailureMessage)(singleResult.acceptance) ?? "Chain acceptance gate rejected the step.";
        writeStatusPayload();
        break;
      }
  ...
  if (maxOutput) {
    const config = { ..._types.DEFAULT_MAX_OUTPUT, ...maxOutput };
    const lastArtifactPath = results[results.length - 1]?.artifactPaths?.outputPath;
    const truncResult = (0, _types.truncateOutput)(summary, config, lastArtifactPath);
  ...
  if (shareEnabled) {
    ...
    if (sessionFile) {
      try {
        ...
        if ("error" in share) shareError = share.error;else
        {
  ...
  const signalTerminated = !stopped && !timedOut && !turnBudgetExceeded && !interrupted && results.some((result) => result.exitCode !== 0 && (0, _processSignal.isUnexplainedProcessSignal)({
    processSignal: result.processSignal,
    interrupted: result.interrupted,
    timedOut: result.timedOut,
    stopped: result.stopped,
  ...
  (0, _controlChannel.closeSteerInbox)(asyncDir, statusPayload.state);
  disposeControlInbox();
  for (const request of (0, _controlChannel.consumeSteerRequests)(asyncDir)) deliverSteerRequest(request);
  const effectiveSessionFile = sessionFile ?? latestSessionFile;
  const steeringLifecycle = (0, _steering.steeringStatus)(statusPayload);
  ...
  if (turnBudgetExceeded && !statusPayload.error) {
    ...
    statusPayload.error = budget ? (0, _turnBudget.turnBudgetExceededMessage)(budget, budget.turnCount) : "Subagent exceeded turn budget.";
  }
  ...
  try {
    (0, _atomicJson.writeAtomicJson)(resultPath, {
      lifecycleArtifactVersion: _types.SUBAGENT_LIFECYCLE_ARTIFACT_VERSION,
      id,
      agent: agentName,
      mode: resultMode,
  ...
  if (config.runnerProcessInstanceId) {
    ...
    try {
      (0, _processTerminal.writeProcessTerminalCandidate)(asyncDir, candidate);
    } catch (error) {
...
async function runConfiguredSubagent(config) {
  ...
  try {
    if (config.revivalLease) {
      lease = (0, _sessionLease.acquireSessionLease)(config.revivalLease);
      config.revivalLeaseToken = lease.owner.token;
      (0, _atomicJson.writeAtomicJson)(startupPath, { state: "ready", token: lease.owner.token, pid: process.pid, owner: lease.owner });
      await waitForStartupControl(startupAckPath, lease.owner.token, "ack");
      (0, _atomicJson.writeAtomicJson)(startupPath, { state: "acknowledged", token: lease.owner.token, pid: process.pid });
    ...
    if (config.revivalLease && !startupCommitted) {
      try {
        (0, _atomicJson.writeAtomicJson)(startupPath, { state: "error", pid: process.pid, error: error instanceof Error ? error.message : String(error) });
      } catch {
    ...
    if (lease) {
      ...
      try {
        (0, _processTerminal.markProcessTerminalCandidateLeaseRelease)(config.asyncDir, lease.owner.token, acknowledged);
      } catch (error) {
