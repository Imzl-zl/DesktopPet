...
> 状态：**已完成（2026-08 全部落地）** · 范围：`windows/` 目录整体替换（已删除，commit 2c9b701）· 保留 macOS 版不变
> 本文是历史路线图：各 Phase 均已实现，代码真名见 `windows-architecture.md` §9；本文件不再维护细节偏差。
...
- `PetAgent`：`Windows.Graphics.Capture` 采集（1fps 节流）→ 帧哈希变化检测 → 仅变化时调多模态（**分析开关**）；云端调用限频 = 分析间隔（3-30s，默认 5s）
...
- **输出模式四选一（弹幕 / 对话 / 气泡 / 静默）**，默认**气泡**：弹幕层（Win2D，事件驱动刷弹幕）与对话气泡窗口均按模式激活创建、切换即销毁；浮球菜单 + 设置页均可切换。**语义**：模式只决定 AI 主动输出形式；用户主动对话随时可开（对话窗不受模式限制）；静默 = 截屏分析停止 + 无主动输出（最干净）
...
- **全局快捷键**：`Win+Ctrl+H` 显隐 / `Win+Ctrl+T` 切换模式 / `Win+Ctrl+U` 设置 / `Win+Ctrl+X` 退出（`RegisterHotKey` P/Invoke；默认避开 Ctrl+Alt，防腾讯会议/QQ 占用）
...
- **语音输出**：对话模式可开启朗读（默认 SAPI 离线合成；音色可升级 OneCore 自然语音 / 用户自配 OpenAI 兼容端点，见 `windows-tts-design.md`），默认关、弹幕模式不朗读；**语音开关**（AI 助手页，默认关）
