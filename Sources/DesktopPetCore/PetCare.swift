...
/// Persistent tamagotchi state: the pet is fed by real work — activities,
/// snacks and finished tasks ("meals").
...
public enum PetCare {
    ...
    /// Directly grants raw XP (activity snacks: chat turns, daily summaries, ...),
    /// keeping daily bookkeeping consistent (rollover + last-fed update). The
    /// pure-pet path for feeding that has no token concept.
    @discardableResult
    public static func addActivityXP(
        _ xp: Int, state: inout PetCareState, now: Date, calendar: Calendar = .current
    ) -> Int {
